...
QPainterPath roundedPanelPath(const QRectF &rect, bool roundTopLeft,
                              bool roundTopRight, bool roundBottomRight,
                              bool roundBottomLeft) {
  const qreal radius =
      qMin<qreal>(CornerRadius, qMin(rect.width(), rect.height()) * 0.5);
  QPainterPath path;
...
public:
  ContentPanel(ExExpander *expander, QWidget *content)
      : QWidget(expander), m_expander(expander) {
    setMinimumHeight(HeaderMinHeight);
    setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Maximum);
    auto *layout = new QVBoxLayout(this);
    layout->setContentsMargins(ContentPadding, ContentPadding, ContentPadding,
                               ContentPadding);
    layout->setSpacing(0);
...
protected:
  void paintEvent(QPaintEvent *) override {
    ...
    const QPainterPath fillPath =
        roundedPanelPath(bounds, !down && m_outerEdge, !down && m_outerEdge,
                         down && m_outerEdge, down && m_outerEdge);
    ...
    if (down) {
      ...
      borderPath.lineTo(bounds.left(),
                        bounds.bottom() - (m_outerEdge ? CornerRadius : 0));
      if (m_outerEdge) {
      ...
      borderPath.lineTo(bounds.right() - (m_outerEdge ? CornerRadius : 0),
                        bounds.bottom());
      if (m_outerEdge) {
        borderPath.quadTo(bounds.right(), bounds.bottom(), bounds.right(),
      ...
      borderPath.lineTo(bounds.left(),
                        bounds.top() + (m_outerEdge ? CornerRadius : 0));
      if (m_outerEdge) {
      ...
      borderPath.lineTo(bounds.right() - (m_outerEdge ? CornerRadius : 0),
                        bounds.top());
      if (m_outerEdge) {
        borderPath.quadTo(bounds.right(), bounds.top(), bounds.right(),
...
public:
  explicit ContentStack(ExExpander *expander) : QWidget(expander) {
...
public:
  ContentViewport(ExExpander *expander, ContentStack *panel)
      : QWidget(expander), m_expander(expander),
        m_clipViewport(new QWidget(this)), m_panel(panel) {
    setMinimumWidth(ExpanderMinWidth);
...
protected:
  ...
  bool eventFilter(QObject *watched, QEvent *event) override {
    if (watched == m_panel && event->type() == QEvent::LayoutRequest &&
        m_expander->m_expansionAnimation->state() !=
...
private:
  int layoutHeight() const {
    return m_expander->isExpanded() ? m_fullHeight
                                    : qRound(m_fullHeight * m_viewportProgress);
  }
...
public:
  explicit HeaderButton(ExExpander *expander)
      : QAbstractButton(expander), m_expander(expander) {
    setCheckable(true);
...
protected:
  ...
  void paintEvent(QPaintEvent *) override {
    ...
    const bool connected =
        m_expander->isExpanded() && m_expander->hasContentWidgets();
    const bool down = m_expander->expandDirection() == ExExpander::Down;
    const QRectF bounds = QRectF(rect()).adjusted(0.5, 0.5, -0.5, -0.5);
    const QPainterPath headerPath =
        roundedPanelPath(bounds, !connected || down, !connected || down,
                         !connected || !down, !connected || !down);
    ...
    const qreal chevronX =
        rtl ? ChevronTrailingMargin
            : width() - ChevronTrailingMargin - ChevronButtonSize;
    const QRectF chevronRect(chevronX, (height() - ChevronButtonSize) * 0.5,
                             ChevronButtonSize, ChevronButtonSize);
    ...
    const QRectF glyphRect(-ChevronButtonSize * 0.5, -ChevronButtonSize * 0.5,
                           ChevronButtonSize, ChevronButtonSize);
...
private:
  void updateLayoutMargins() {
    const int chevronSide =
        ChevronContentSpacing + ChevronButtonSize + ChevronTrailingMargin;
...
ExExpander::ExExpander(QWidget *parent)
    : QWidget(parent), m_rootLayout(new QVBoxLayout(this)),
  ...
  connect(m_expansionAnimation, &QVariantAnimation::finished, this,
          &ExExpander::finishTransition);
}
...
ExExpander::~ExExpander() {
  ...
  for (const QMetaObject::Connection &connection :
       m_contentWidgetDestroyedConnections) {
    disconnect(connection);
  }
}

QString ExExpander::header() const { return m_header; }
...
bool ExExpander::isExpanded() const { return m_expanded; }
...
bool ExExpander::isAnimationEnabled() const { return m_animationEnabled; }
...
void ExExpander::setAnimationEnabled(bool enabled) {
  ...
  if (!enabled &&
      m_expansionAnimation->state() == QAbstractAnimation::Running) {
...
int ExExpander::animationDuration() const { return m_animationDuration; }
...
QWidget *ExExpander::headerWidget() const { return m_headerWidget.data(); }

void ExExpander::setHeaderWidget(QWidget *widget) {
  if (widget == this || widget == m_headerButton ||
      widget == m_contentContainer || widget == m_contentStack ||
      isInternalWidget(widget) || isContentWidgetOrDescendant(widget) ||
  ...
  m_headerWidget = widget;
  if (widget) {
    m_headerWidgetDestroyedConnection =
...
void ExExpander::addContentWidget(QWidget *widget) {
  if (!widget || widget == this || widget == m_headerButton ||
      widget == m_contentContainer || widget == m_contentStack ||
...
bool ExExpander::isInternalWidget(const QWidget *widget) const {
  return widget && (m_headerButton->isAncestorOf(widget) ||
...
bool ExExpander::isContentWidgetOrDescendant(const QWidget *widget) const {
  ...
  for (const QPointer<QWidget> &content : m_contentWidgets) {
    if (content &&
        (content.data() == widget || content->isAncestorOf(widget))) {
      return true;
    }
  }
  return false;
...
void ExExpander::refreshContentGeometry() {
  ...
  m_contentContainer->prepareAnimation();
  m_contentContainer->setViewportProgress(m_expansionProgress);
  const bool visible = (m_expanded || m_expansionAnimation->state() ==
                                          QAbstractAnimation::Running) &&
                       hasContentWidgets();
...
QSize ExExpander::sizeHint() const { return m_rootLayout->sizeHint(); }
...
void ExExpander::toggle() { setExpanded(!m_expanded); }
