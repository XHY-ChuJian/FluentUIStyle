...
static int cmd_configure(const SCL::ParseResult &result) {
    ...
    if (const auto &warningResult = result.option("-w"); warningResult.isSet()) {
        do {
            ...
            std::ifstream inFile(qm_stream_path(fs::path(warningFile)));
            if (!inFile.is_open()) {
                break;
            }
    ...
    do {
        ...
        std::ifstream inFile(qm_stream_path(fileName));
        if (!inFile.is_open()) {
            break;
        }
    ...
    // Create file
    {
        ...
        std::ofstream outFile(qm_stream_path(fileName));
        if (!outFile.is_open()) {
    ...
    for (const auto &entry : fs::recursive_directory_iterator(src)) {
        if (fs::is_regular_file(entry.path())) {
            ...
            if (copy) {
                ...
                // Create file
                std::ofstream outFile(qm_stream_path(targetPath));
                if (!outFile.is_open()) {
...
static int cmd_deploy(const SCL::ParseResult &result) {
    ...
    std::vector<fs::path> searchingPaths;
    {
        ...
        for (const auto &item : configFiles) {
            std::ifstream file(qm_stream_path(fs::path(str2tstr(item.toString()))),
                               std::ios::binary);
            if (!file.is_open()) {
                continue;
            }
