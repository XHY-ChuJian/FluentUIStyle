...
inline QDate cellDateFromIndex(const QCalendarWidget *calendar,
  ...
  const int firstRow = (calendar->horizontalHeaderFormat() !=
  ...
  const int colForFirstOfMonth =
      (columnForDayOfWeek(static_cast<Qt::DayOfWeek>(refDate.dayOfWeek())) -
       (refDate.day() % 7) + 8) %
      7;
  ...
  const int requestedDay =
      7 * (row - firstRow) + column - colForFirstOfMonth - refDate.day() + 1;
...
public:
  explicit FluentCalendarDelegate(QCalendarWidget *calendar,
  ...
  void paint(QPainter *painter, const QStyleOptionViewItem &option,
    ...
    const bool isToday =
        cellDate.isValid() && (cellDate == QDate::currentDate());
    const bool isSelected =
        (option.state & QStyle::State_Selected) ||
        (cellDate.isValid() && cellDate == m_calendar->selectedDate());
    const bool isCurrentMonth =
        cellDate.isValid() && (cellDate.month() == m_calendar->monthShown()) &&
        (cellDate.year() == m_calendar->yearShown());
    ...
    const QRectF circleRect(r.center().x() - diameter / 2.0,
                            r.center().y() - diameter / 2.0, diameter,
                            diameter);
    ...
    if (!text.isEmpty()) {
      ...
      const bool isWeekend =
          cellDate.isValid() && (cellDate.dayOfWeek() == Qt::Saturday ||
                                 cellDate.dayOfWeek() == Qt::Sunday);
...
public:
  explicit FluentCalendarEventFilter(QCalendarWidget *calendar)
      : QObject(calendar), m_calendar(calendar) {}
  ...
  bool eventFilter(QObject *watched, QEvent *event) override {
    if (watched == m_calendar) {
      if (event->type() == QEvent::Paint) {
        ...
        const QRect panelRect = m_calendar->rect().adjusted(
            shadowReserve, shadowReserve, -shadowReserve, -shadowReserve);
        ...
        qApp->style()->drawPrimitive(
            static_cast<QStyle::PrimitiveElement>(PE_FluentFlyoutSurface), &opt,
            &painter, m_calendar);
        painter.restore();
      ...
      if ((isPrev || isNext || isMonth || isYear) &&
        ...
        if (isPrev || isNext) {
          ...
          if (auto *style = qobject_cast<FluentUI3Style *>(qApp->style())) {
            opt.icon =
                style->fluentIcon(QChar(isPrev ? 0xEDDB : 0xEDDC), iconColor);
            opt.iconSize =
                btn->iconSize().isValid() ? btn->iconSize() : QSize(14, 14);
          }
        }
...
void polishCalendarWidget(QCalendarWidget *calendar) {
  ...
  calendar->setMinimumSize(320 + FlyoutShadowBorderWidth * 2,
                           360 + FlyoutShadowBorderWidth * 2);
  ...
  // 定制顶部导航栏及其子控件
  auto *prevBtn = calendar->findChild<QToolButton *>(
      QStringLiteral("qt_calendar_prevmonth"));
  auto *nextBtn = calendar->findChild<QToolButton *>(
      QStringLiteral("qt_calendar_nextmonth"));
  auto *monthBtn = calendar->findChild<QToolButton *>(
      QStringLiteral("qt_calendar_monthbutton"));
  auto *yearBtn = calendar->findChild<QToolButton *>(
      QStringLiteral("qt_calendar_yearbutton"));
  auto *yearEdit =
      calendar->findChild<QSpinBox *>(QStringLiteral("qt_calendar_yearedit"));
  auto *navBar = calendar->findChild<QWidget *>(
  ...
  // 重构顶部导航栏为标准 WinUI 3 布局：[月份] [年份] [弹性拉伸] [上一月] [下一月]
  // 保证月年紧挨并排在最左侧，弹性弹簧位于年份与翻页按钮之间
  if (navBar) {
  ...
  // 定制内部表格视图 QTableView
  if (auto *view = calendar->findChild<QTableView *>(
