...
// GPL-2.0-only OR GPL-3.0-only Qt-Security score:significant reason:default
...
inline QPixmap styleCachePixmap(const QSize& size)
{
    ...
    const QSize pixelSize = size * pixelRatio;

    QImage img(pixelSize, QImage::Format_ARGB32_Premultiplied);
    img.fill(Qt::transparent);

    QPixmap cachePixmap = QPixmap::fromImage(img);
                    ...
                    qreal dpr )
{
    ...
    QString tmp =
        key % HexString<uint>( option->state )
        % HexString<uint>( option->direction )
        % HexString<uint>(
            complexOption ? uint( complexOption->activeSubControls ) : 0u )
        % HexString<quint64>( option->palette.cacheKey() )
        % HexString<uint>( size.width() ) % HexString<uint>( size.height() )
        % HexString<qreal>( dpr );
...
static QPointF calcRadialPos( const QStyleOptionSlider* dial, qreal offset )
{
    ...
    {
        a = Q_PI * 3 / 2
            - ( currentSliderPosition - dial->minimum ) * 2 * Q_PI
                  / ( dial->maximum - dial->minimum );
    }
    else
    {
        a = ( Q_PI * 8
              - ( currentSliderPosition - dial->minimum ) * 10 * Q_PI
                    / ( dial->maximum - dial->minimum ) )
            / 6;
    }
    qreal xc   = width / 2.0;
...
QPolygonF calcLines( const QStyleOptionSlider* dial )
{
    ...
    {
        qreal angle = dial->dialWrapping
                          ? Q_PI * 3 / 2 - i * 2 * Q_PI / notches
                          : ( Q_PI * 8 - i * 10 * Q_PI / notches ) / 6;
        qreal s     = qSin( angle );
        qreal c     = qCos( angle );
        if ( i == 0
             || ( ( ( ns * i ) % ( dial->pageStep ? dial->pageStep : 1 ) )
                  == 0 ) )
        {
...
void drawDial( const QStyleOptionSlider* option, QPainter* painter )
{
    ...
    {
        const bool inverted =
            pal.window().color().lightness() < pal.text().color().lightness()
    ...
    if ( p.needsPainting() )
    {
        ...
        if ( enabled )
        {
            ...
            QRectF shadowRect = br.adjusted( -2 * shadowSize,
                                             -2 * shadowSize,
                                             2 * shadowSize,
                                             2 * shadowSize );
            ...
            // Main gradient
            QRadialGradient gradient( br.center().x() - br.width() / 3,
                                      dy,
                                      br.width() * 1.3,
                                      br.center().x(),
    ...
    if ( penSize > 3.0 )
    {
        ...
        painter->drawLine( calcRadialPos( option, qreal( 0.90 ) ),
                           calcRadialPos( option, qreal( 0.96 ) ) );
    }
...
void drawBorderPixmap( const QPixmap& pixmap,
                       QPainter* painter,
                       const QRect& rect,
                       int left,
                       int top,
                       ...
                       int bottom )
{
    ...
    if ( top > 0 )
    {
        ...
        if ( left > 0 )
        {
            painter->drawPixmap( QRect( rect.left(), rect.top(), left, top ),
                                 pixmap,
                                 QRect( 0, 0, left, top ) );
        }
        ...
        if ( right > 0 )
        {
            painter->drawPixmap( QRect( rect.left() + rect.width() - right,
                                        rect.top(),
                                        right,
                                        top ),
                                 pixmap,
    ...
    // center
    painter->drawPixmap( QRect( rect.left() + left,
                                rect.top() + top,
                                rect.width() - right - left,
...
QColor backgroundColor( const QPalette& pal, const QWidget* widget )
{
    ...
    {
        return widget->parentWidget()->parentWidget()->palette().color(
            QPalette::Base );
    }
#else
    Q_UNUSED( widget );
...
WidgetSizePolicy widgetSizePolicy( const QWidget* widget,
