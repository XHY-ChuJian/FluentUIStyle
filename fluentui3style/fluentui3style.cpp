...
static constexpr int topLevelRoundingRadius =
    6; // Radius for toplevel items like popups for round corners
static int secondLevelRoundingRadius =
    4; // Radius for second level items like hovered menu item round corners
static constexpr int contentItemHMargin =
    4; // margin between content items (e.g. text and icon)
static constexpr int contentHMargin =
    2 * 3; // margin between rounded border and content (= rounded border
           // margin * 3)
static constexpr int pivotIndicatorPreferredWidth =
    24; // Pivot_Grow / Slide / Stretch: fixed bar length, centered
...
static constexpr const char *cBPopupAnimatorProperty =
    "_q_fluent_combo_popup_animator";
static constexpr int menuPopupAnimationDuration = 400;
static constexpr const char *menuPopupAnimatorProperty =
    "_q_fluent_menu_popup_animator";
...
static QMarginsF comboBoxPopupPanelMargins(const QWidget *popup) {
  Q_UNUSED(popup)
  return QMarginsF(FlyoutShadowBorderWidth, FlyoutShadowBorderWidth,
                   FlyoutShadowBorderWidth, FlyoutShadowBorderWidth);
}
...
public:
  explicit SliderValueTipLabel(QWidget *parent) : QLabel(parent) {
...
static void initSliderStyleOption(const QSlider *slider,
                                  QStyleOptionSlider *opt, int position) {
  opt->initFrom(slider);
...
static SliderValueTipLabel *sliderValueTip(QSlider *slider) {
  QObject *const obj =
      slider->property(SliderValueTipLabelProperty).value<QObject *>();
...
static void showSliderValueTip(QSlider *slider, int value) {
  if (!sliderValueTipEnabled(slider) || !slider->isEnabled() ||
      !slider->isVisible()) {
    return;
  }
  ...
  if (!tip) {
    ...
    slider->setProperty(SliderValueTipLabelProperty,
                        QVariant::fromValue<QObject *>(tip));
  ...
  const QPoint globalCenter =
      slider->mapToGlobal(QRectF(handleRect).center().toPoint());
  int x = 0;
  int y = 0;
  if (slider->orientation() == Qt::Horizontal) {
    x = globalCenter.x() - tip->width() / 2;
    y = globalCenter.y() - static_cast<int>(outerRadius) -
        kSliderValueTipGapAboveHandle - tip->height();
  } else {
    x = globalCenter.x() - static_cast<int>(outerRadius) -
        kSliderValueTipGapAboveHandle - tip->width();
...
static void installSliderValueTipHooks(QSlider *slider) {
  ...
  QObject::connect(slider, &QSlider::sliderPressed, slider, [slider]() {
    showSliderValueTip(slider, slider->sliderPosition());
  });
  QObject::connect(slider, &QSlider::sliderMoved, slider, [slider](int value) {
    if (QApplication::mouseButtons() & Qt::LeftButton) {
      showSliderValueTip(slider, value);
    }
  });
  QObject::connect(slider, &QSlider::sliderReleased, slider,
                   [slider]() { hideSliderValueTip(slider); });
}

static QColor segmentedColorFromVariant(const QVariant &v,
...
static QColor segmentedThemeColorProperty(const QWidget *widget, bool darkTheme,
...
static bool segmentedBoolProperty(const QWidget *widget, const char *name,
                                  bool fallback) {
  if (!widget) {
    return fallback;
  }
...
void startAnimationEx(QStyleAnimation *animation, QObject *target,
                      const QByteArray &key);

static QByteArray navigationSettingsAnimKey(const QTreeView *treeView,
...
static QPixmap navigationGlyphPixmapCached(const QString &iconCode,
                                           int glyphSide, const QColor &color) {
  static QHash<QString, QPixmap> cache;
  const QString cacheKey = iconCode + QLatin1Char('|') +
                           QString::number(glyphSide) + QLatin1Char('|') +
...
static void startNavigationSettingsSpin(QObject *target,
                                        ...
                                        const QModelIndex &index,
                                        qreal deltaDegrees, int durationMs) {
  ...
  qreal start = 0.0;
  if (QNumberStyleAnimation *existAnim = qobject_cast<QNumberStyleAnimation *>(
          getAnimationEx(target, animKey))) {
...
static inline void drawRoundedRect(QPainter *p, R &&rect, P &&pen, B &&brush) {
  p->setPen(pen);
  p->setBrush(brush);
  p->drawRoundedRect(rect, secondLevelRoundingRadius,
                     secondLevelRoundingRadius);
}

// From QCommonStylePrivate::tabLayout
static inline void tabLayout(const QStyle *proxyStyle,
                             const QStyleOptionTab *opt, const QWidget *widget,
                             QRect *textRect, QRect *iconRect) {
  Q_ASSERT(textRect);
  Q_ASSERT(iconRect);
  QRect tr = opt->rect;
  const bool isNavigationStyle =
      widget &&
      widget->property(TabBarStyleProperty).toInt() == TabBarStyle::Navigation;
  const bool isVerticalNavigation =
      isNavigationStyle && (opt->shape == QTabBar::RoundedEast ||
  ...
  if (isVerticalNavigation) {
    ...
    tr.adjust(contentLeadingInset, listItemVerticalMargin,
              -contentTrailingInset, -listItemVerticalMargin);
    ...
    if (!opt->icon.isNull()) {
      ...
      if (!iconSize.isValid()) {
        const int iconExtent =
            proxyStyle->pixelMetric(QStyle::PM_SmallIconSize, opt, widget);
      ...
      *iconRect = QStyle::visualRect(opt->direction, opt->rect, *iconRect);
      ...
      int cachedIconWidth =
          widget ? widget->property("_q_navigation_widest_icon_width").toInt()
                 : 0;
  ...
  int verticalShift =
      proxyStyle->pixelMetric(QStyle::PM_TabBarTabShiftVertical, opt, widget);
  int horizontalShift =
      proxyStyle->pixelMetric(QStyle::PM_TabBarTabShiftHorizontal, opt, widget);
  int hpadding =
      proxyStyle->pixelMetric(QStyle::PM_TabBarTabHSpace, opt, widget) / 2;
  int vpadding =
      proxyStyle->pixelMetric(QStyle::PM_TabBarTabVSpace, opt, widget) / 2;
...
static inline QPainterPath buildRoundedPolyline(const QList<QPointF> &points,
                                                qreal radius) {
  QPainterPath path;
...
static qreal radioButtonInnerRadius(int state, const QStyleOption *option,
                                    const QWidget *widget, int indicatorSize) {
  Q_UNUSED(widget)
  Q_UNUSED(option)
...
static QSizeF viewItemTextLayout(QTextLayout &textLayout, int lineWidth,
  ...
  while (true) {
    ...
    // one
    if (maxHeight > 0 && lastVisibleLine &&
        height + line.height() > maxHeight) {
...
QString calculateElidedText(const QString &text, const QTextOption &textOption,
  ...
  const bool vAlignmentOptimization =
      paintStartPosition && valign.testFlag(Qt::AlignVCenter);
  ...
  const QRect layoutRect = QStyle::alignedRect(
      Qt::LayoutDirectionAuto, valign, boundingRect.size().toSize(), textRect);
...
void viewItemDrawText(QPainter *p, const QStyleOptionViewItem *option,
                      const QRect &rect) {
  const QWidget *widget = option->widget;
  const int textMargin = qApp->style()->pixelMetric(
                             QStyle::PM_FocusFrameHMargin, nullptr, widget) +
                         1;

  QRect textRect =
      rect.adjusted(textMargin, 0, -textMargin, 0); // remove width padding
  const bool wrapText = option->features & QStyleOptionViewItem::WrapText;
  QTextOption textOption;
  textOption.setWrapMode(wrapText ? QTextOption::WordWrap
  ...
  const QString newText = calculateElidedText(
      option->text, textOption, option->font, textRect,
      option->displayAlignment, option->textElideMode, 0, true, &paintPosition);
...
QString toolButtonElideText(const QStyleOptionToolButton *option,
  ...
  return calculateElidedText(text, textOption, option->font, textRect,
                             Qt::AlignTop, Qt::ElideMiddle, flags, false,
                             nullptr);
}
...
template <typename K, typename V> V takeValue(QHash<K, V> &h, K key) {
  V val = h.value(key);
  h.remove(key);
  return val;
}

void removeAnimation(QObject *target) { animations.remove(target); }
...
void removeAnimationEx(QObject *target, const QByteArray &key,
                       QStyleAnimation *animation) {
  auto it = extraAnimations.find(target);
  if (it == extraAnimations.end()) {
    return;
  }
...
void startAnimationEx(QStyleAnimation *animation, QObject *target,
  ...
  if (!extraAnimations.contains(target)) {
    QObject::connect(target, &QObject::destroyed,
                     [](QObject *obj) { clearAnimationsEx(obj); });
  }
...
float animationValue(QObject *target, const QByteArray &key,
                     float defaultValue) {
  auto anim =
      qobject_cast<QNumberStyleAnimation *>(getAnimationEx(target, key));
...
bool transitionsEnabled() { return true; }
...
static void drawRoundedUnderline(QPainter *painter, const QRectF &shapeRect,
                                 const QRectF &frameRect, qreal radius,
                                 const QPen &pen) {
  PainterStateGuard guard(painter);
  const QRectF underlineClip(frameRect.left(), frameRect.bottom() - 1.0,
                             frameRect.width(), 3.0);
...
static void drawRoundedBorderSurface(QPainter *painter, const QRectF &rect,
                                     qreal radius, const QBrush &background,
                                     const QBrush &border, qreal borderWidth,
  ...
  const qreal width =
      qBound(0.0, borderWidth, qMin(rect.width(), rect.height()) / 2.0);
...
inline void qDrawPlainRoundedRect(QPainter *p, int x, int y, int w, int h,
...
inline void qDrawPlainRoundedRect(QPainter *painter, const QRect &rect,
                                  qreal rx, qreal ry, const QColor &lineColor,
                                  int lineWidth = 1,
                                  const QBrush *fill = nullptr) {
  qDrawPlainRoundedRect(painter, rect.x(), rect.y(), rect.width(),
                        rect.height(), rx, ry, lineColor, lineWidth, fill);
}

void qDrawShadeRect(QPainter *p, int x, int y, int w, int h,
                    const QPalette &pal, bool sunken = false, int lineWidth = 1,
                    int midLineWidth = 0, const QBrush *fill = nullptr) {
  if (w == 0 || h == 0) {
    return;
  }
  ...
  if (lineWidth == 1 && midLineWidth == 0) { // standard shade rectangle
    ...
    QLineF lines[4] = {QLineF(x1 + 1, y1 + 1, x2 - 2, y1 + 1),
                       ...
                       QLineF(x1, y2, x2, y2), QLineF(x2, y1, x2, y2 - 1)};
...
void qDrawShadeRect(QPainter *p, const QRect &r, const QPalette &pal,
                    bool sunken = false, int lineWidth = 1,
                    int midLineWidth = 0, const QBrush *fill = nullptr) {
  qDrawShadeRect(p, r.x(), r.y(), r.width(), r.height(), pal, sunken, lineWidth,
                 midLineWidth, fill);
}
...
void qDrawShadePanel(QPainter *p, int x, int y, int w, int h,
  ...
  if (fill) { // fill with fill color
    ...
    p->fillRect(x + lineWidth, y + lineWidth, w - lineWidth * 2,
                h - lineWidth * 2, *fill);
  }
}

void qDrawShadePanel(QPainter *p, const QRect &r, const QPalette &pal,
                     bool sunken = false, int lineWidth = 1,
                     const QBrush *fill = nullptr) {
...
#else
    if (auto screen =
            QGuiApplication::screenAt(w->mapToGlobal(w->rect().center())))
#endif
    {
      return screen;
    }
...
static bool updateBrushOrigin_public(QPainter *painter, const QWidget *widget,
                                     const QBrush &brush) {
  if (!widget) {
    return false;
  }
  ...
  auto scrollArea =
      qobject_cast<const QAbstractScrollArea *>(viewport->parentWidget());
  ...
  const QPoint offset = scrollArea->horizontalScrollBar()
                            ? QPoint(scrollArea->horizontalScrollBar()->value(),
...
inline int getColorSchemeIndex() // 0 = Light, 1 = Dark
{
  ...
  qDebug() << "[FluentUI3Style] No set _q_colorscheme, use system theme color"
...
FluentUI3Style::FluentUI3Style(QStyle *style)
  ...
  if (qApp->property("secondLevelRoundingRadius").isValid()) {
    secondLevelRoundingRadius =
        qApp->property("secondLevelRoundingRadius").toInt();
    qDebug() << "[FluentUI3Style] set secondLevelRoundingRadius:"
             << secondLevelRoundingRadius;
  }
...
void FluentUI3Style::drawComplexControl(ComplexControl control,
  ...
  const auto drawTitleBarButton = [&](ComplexControl control, SubControl sc,
                                      const QString &str) {
    using namespace StyleOptionHelper;
    const QRect buttonRect =
        proxy()->subControlRect(control, option, sc, widget);
  ...
  const auto drawTitleBarCloseButton = [&](ComplexControl control,
                                           SubControl sc, const QString &str) {
    using namespace StyleOptionHelper;
    const QRect buttonRect =
        proxy()->subControlRect(control, option, sc, widget);
    if (buttonRect.isValid()) {
      const auto state = (option->activeSubControls == sc)
                             ? calcControlState(option)
                             : ControlState::Normal;
      QPen pen;
      switch (state) {
  ...
  if (transitionsEnabled() && option->styleObject) {
    if (control == CC_Slider) {
      if (const auto *slider =
        ...
        QObject *styleObject =
            option->styleObject; // Can be widget or qquickitem

        const qreal outerRadius = sliderHandleDiameter / 2.0 - 1.0;
        bool isInsideHandle = option->activeSubControls == SC_SliderHandle;

        bool oldIsInsideHandle =
            styleObject->property("_q_insidehandle").toBool();
        State oldState = State(styleObject->property("_q_stylestate").toInt());
        SubControls oldActiveControls =
            SubControls(styleObject->property("_q_stylecontrols").toInt());
        ...
        styleObject->setProperty("_q_stylecontrols",
        ...
        if (doTransition) {
          ...
          t->setEndValue(outerRadius *
                         sliderInnerRadius(state, isInsideHandle));
    ...
    if (control == CC_ComboBox) {
      ...
      if (doTransition && (state & State_Enabled)) {
        ...
        if (!widget->inherits("ExComboBox") && !(state & State_On) &&
            (oldState & State_On)) {
          t->setDuration(180);
        }
        startAnimation(t);
  ...
  case CC_SpinBox:
    if (const QStyleOptionSpinBox *sb =
      ...
      SpinBoxButtonLayout buttonLayout =
          widget ? static_cast<SpinBoxButtonLayout>(
                       widget->property("spinBoxButtonLayout").toInt())
      ...
      const bool isEditable =
          spinBox ? !spinBox->isReadOnly() : !(state & State_ReadOnly);

      QCachedPainter cp(painter,
                        QLatin1String("win11_spinbox") %
                            HexString<uint8_t>(colorSchemeIndex) %
                            HexString<uint8_t>(buttonLayout),
                        sb, sb->rect.size());
      if (cp.needsPainting()) {
        if (sb->frame && (sub & SC_SpinBoxFrame)) {
          ...
          const QColor bg =
              resolveOpaque(winUI3Color(fillColor), sb->palette.base().color());
          drawRoundedRect(cp.painter(), frameRect, Qt::NoPen, bg);

          proxy()->drawPrimitive(PE_FrameLineEdit, &panel, cp.painter(),
                                 widget);
        }
        ...
        if (isMouseOver && !hasFocus && !highContrastTheme) {
          drawRoundedRect(cp.painter(), frameRect, Qt::NoPen,
        ...
        const auto drawUpDown = [&](QStyle::SubControl sc) {
          ...
          const QRect rect =
              proxy()->subControlRect(CC_SpinBox, option, sc, widget);
          if (sb->activeSubControls & sc) {
            drawRoundedRect(cp.painter(), rect.adjusted(1, 1, -1, -2),
                            Qt::NoPen, winUI3Color(subtleHighlightColor));
          }
        ...
        if (state & State_KeyboardFocusChange && state & State_HasFocus) {
          ...
          proxy()->drawPrimitive(PE_FrameFocusRect, &fropt, cp.painter(),
                                 widget);
        }
      }
      cp.finish();
      ...
      if (sb->frame && (sub & SC_SpinBoxFrame) && isEditable) {
        ...
        const auto underlineCol =
            hasFocus ? accentColor(option)
                     : (colorSchemeIndex == 0 ? QColor(0x80, 0x80, 0x80)
                                              : QColor(0xa0, 0xa0, 0xa0));
        drawRoundedUnderline(painter, frameRect, frameRect,
                             secondLevelRoundingRadius,
  ...
  case CC_Slider:
    if (const auto *slider =
      ...
      if (sub & SC_SliderGroove) {
        ...
        if (isHorizontal) {
          rect = QRectF(rect.left() + 2, rect.center().y() - 2,
                        rect.width() - 2, 4);
          leftRect = QRectF(rect.left(), rect.top(),
                            handleCenter.x() - rect.left(), rect.height());
          rightRect = QRectF(handleCenter.x(), rect.top(),
                             rect.width() - handleCenter.x(), rect.height());
        } else {
          rect = QRect(rect.center().x() - 2, rect.top() + 2, 4,
                       rect.height() - 2);
      ...
      if (sub & SC_SliderTickmarks) {
        int tickOffset =
            proxy()->pixelMetric(PM_SliderTickmarkOffset, slider, widget);
        int ticks = slider->tickPosition;
        int thickness =
            proxy()->pixelMetric(PM_SliderControlThickness, slider, widget);
        ...
        while (v <= slider->maximum + 1) {
          ...
          int tickLength =
              (v_ == slider->minimum || v_ >= slider->maximum) ? 4 : 3;
          int pos = QStyle::sliderPositionFromValue(
              slider->minimum, slider->maximum, v_, available,
              slider->upsideDown);
          pos += fudge;
          if (isHorizontal) {
            if (ticks & QSlider::TicksAbove) {
              lines.append(QLineF(pos, tickOffset - 0.5, pos,
                                  tickOffset - tickLength - 0.5));
            }

            if (ticks & QSlider::TicksBelow) {
              lines.append(QLineF(pos, tickOffset + thickness + 0.5, pos,
      ...
      if (sub & SC_SliderHandle) {
        ...
        if (option->styleObject) {
          const QNumberStyleAnimation *animation =
              qobject_cast<QNumberStyleAnimation *>(
                  getAnimation(option->styleObject));
          if (animation != nullptr) {
            ...
            bool isInsideHandle = option->activeSubControls == SC_SliderHandle;
            innerRadius =
                outerRadius * sliderInnerRadius(state, isInsideHandle);
          }
        }
        painter->setPen(Qt::NoPen);
        ...
        painter->setBrush(Qt::NoBrush);
        painter->drawEllipse(handleCenter, outerRadius + 0.5,
                             outerRadius + 0.5);
      }
  ...
  case CC_ComboBox: {
    if (const QStyleOptionComboBox *combobox =
      ...
      const QRectF frameRect =
          QRectF(option->rect)
              .marginsRemoved(
                  QMarginsF(cBShadowBorderWidth, 1, cBShadowBorderWidth, 1));
      QStyleOption opt(*option);
      opt.state.setFlag(QStyle::State_On, false);

      auto drawRoundedRect = [](QPainter *p, QRectF rect, QPen pen,
                                QBrush brush) {
        p->setPen(pen);
        p->setBrush(brush);
        p->drawRoundedRect(rect, secondLevelRoundingRadius,
      ...
      if (combobox->frame) {
        ...
        const auto frameCol =
            highContrastTheme
                ...
                option->palette.color(isHovered ? QPalette::Accent
                                                : QPalette::ButtonText)
#else
                option->palette.color(
                    isHovered ? QPalette::Highlight
      ...
      if (sub & SC_ComboBoxArrow) {
        ...
        QRect r(-arrowRect.width() / 2, -arrowRect.height() / 2,
  ...
  case CC_ScrollBar:
    if (const QStyleOptionSlider *scrollbar =
      ...
      if (transitionsEnabled() && scrollbar->styleObject) {
        ...
        if (QNumberStyleAnimation *animation =
                qobject_cast<QNumberStyleAnimation *>(
                    getAnimation(styleObject))) {
      ...
      if (cp.needsPainting()) {
        ...
        if (progress > 0.0) {
          ...
          cp->setPen(Qt::NoPen);
          cp->drawRoundedRect(rect, topLevelRoundingRadius,
          ...
          cp->drawRoundedRect(rect, topLevelRoundingRadius + 0.5,
                              topLevelRoundingRadius + 0.5);
        }
        if (sub & SC_ScrollBarSlider) {
          QRectF rect = proxy()->subControlRect(CC_ScrollBar, option,
          ...
          cp->setPen(Qt::NoPen);
          qreal cornerRadius =
              _width / 2 + (secondLevelRoundingRadius - _width / 2) * progress;
        ...
        if (sub & SC_ScrollBarSubLine) {
          if (progress > 0.0) {
            const QRectF rect = proxy()->subControlRect(
                CC_ScrollBar, option, SC_ScrollBarSubLine, widget);
            QFont f = QFont(assetFont);
            f.setPointSize(6);
            cp->setFont(f);
            QColor penColor = Qt::gray;
  ...
  case CC_TitleBar:
    if (const auto *titlebar =
      ...
      QColor textColor = titlebar->palette.color(
          titlebar->titleBarState & Qt::WindowActive ? QPalette::Active
                                                     : QPalette::Disabled,
          QPalette::WindowText);
      ...
      // min button
      if (shouldDrawButton(SC_TitleBarMinButton,
      ...
      // max button
      if (shouldDrawButton(SC_TitleBarMaxButton,
      ...
      if (shouldDrawButton(SC_TitleBarCloseButton, Qt::WindowSystemMenuHint)) {
        drawTitleBarCloseButton(CC_TitleBar, SC_TitleBarCloseButton,
                                ChromeClose);
      }
      ...
      // context help button
      if (shouldDrawButton(SC_TitleBarContextHelpButton,
      ...
      // unshade button
      if (shouldDrawButton(SC_TitleBarUnshadeButton,
                           Qt::WindowShadeButtonHint)) {
        drawTitleBarButton(CC_TitleBar, SC_TitleBarUnshadeButton,
                           ChevronDownSmall);
      }

      // window icon for system menu
      if (shouldDrawButton(SC_TitleBarSysMenu, Qt::WindowSystemMenuHint)) {
        const QRect iconRect = proxy()->subControlRect(
            CC_TitleBar, titlebar, SC_TitleBarSysMenu, widget);
        if (iconRect.isValid()) {
          if (!titlebar->icon.isNull()) {
            ...
            const auto extent =
                proxy()->pixelMetric(PM_SmallIconSize, &tool, widget);
            const auto dpr = QStyleHelper::getDpr(widget);
            const auto icon =
                proxy()->standardIcon(SP_TitleBarMenuButton, &tool, widget);
  ...
  case CC_ToolButton:
    if (const auto *toolbutton =
      ...
      button =
          proxy()->subControlRect(control, toolbutton, SC_ToolButton, widget);
      menuarea = proxy()->subControlRect(control, toolbutton, SC_ToolButtonMenu,
                                         widget);
      ...
      if (toolbutton->state & State_HasFocus) {
        ...
        if (toolbutton->features & QStyleOptionToolButton::MenuButtonPopup) {
          fr.rect.adjust(0, 0,
                         -proxy()->pixelMetric(QStyle::PM_MenuButtonIndicator,
                                               toolbutton, widget),
                         0);
        }
      ...
      if (toolbutton->subControls & SC_ToolButtonMenu) {
        ...
        tool.state = mflags;
        if (autoRaise) {
          proxy()->drawPrimitive(PE_IndicatorButtonDropDown, &tool, painter,
                                 widget);
        } else {
          tool.state = mflags;
          ...
          if ((bflags & State_Sunken) != (mflags & State_Sunken)) {
            ...
            tool.rect = option->rect;
            proxy()->drawPrimitive(PE_PanelButtonBevel, &tool, painter,
                                   nullptr);
            painter->restore();
          }
          // Draw arrow
  ...
  case CC_Dial: {
    ...
    if (dialStyle == DialDots || dialStyle == DialRing ||
        dialStyle == DialThumb) {
...
void FluentUI3Style::drawSpecialButton(QPainter *painter,
  ...
  } else if (objectName == "win_caption_minimize" ||
             objectName == "win_caption_maximize" ||
             objectName == "win_caption_theme" ||
  ...
  } else if (objectName == "ScrollLeftButton" ||
...
void FluentUI3Style::drawPrimitive(PrimitiveElement element,
  ...
  if (transitionsEnabled() && option->styleObject &&
    ...
    if (doTransition) {
      if (element == PE_IndicatorRadioButton) {
        QNumberStyleAnimation *t = new QNumberStyleAnimation(styleObject);
        t->setStartValue(styleObject->property("_q_inner_radius").toFloat());
        int indicatorSize = proxy()->pixelMetric(
            QStyle::PM_ExclusiveIndicatorWidth, option, widget);
        t->setEndValue(
            radioButtonInnerRadius(state, option, widget, indicatorSize));
        styleObject->setProperty("_q_end_radius", t->endValue());
        t->setDuration(150);
        startAnimation(t);
      } else if (element == PE_IndicatorCheckBox && widget &&
                 widget->property(SwitchStyleProperty).toBool() == false) {
        if ((oldState & State_Off && state & State_On) ||
            (oldState & State_NoChange && state & State_On)) {
          QNumberStyleAnimation *t = new QNumberStyleAnimation(styleObject);
          ...
          startAnimation(t);
                 ...
                 widget->property(SwitchStyleProperty).toBool() == true) {
        if ((state & State_On) != (oldState & State_On)) {
          QNumberStyleAnimation *t = new QNumberStyleAnimation(styleObject);
          t->setStartValue(animationValue(styleObject, "_q_thumb_pos",
                                          oldState & State_On ? 1.0f : 0.0f));
        ...
        if ((state & State_MouseOver) != (oldState & State_MouseOver)) {
          QNumberStyleAnimation *t = new QNumberStyleAnimation(styleObject);
          t->setStartValue(
              animationValue(styleObject, "_q_thumb_scale",
                             oldState & State_MouseOver ? 1.1f : 0.9f));
        ...
        if ((state & State_Sunken) != (oldState & State_Sunken)) {
          QNumberStyleAnimation *t = new QNumberStyleAnimation(styleObject);
          t->setStartValue(
              animationValue(styleObject, "_q_thumb_stretch",
                             oldState & State_Sunken ? 1.3f : 1.0f));
  ...
  case PE_PanelTipLabel: {
    ...
    panelRect.adjust(shadowReserve, shadowReserve, -shadowReserve,
                     -shadowReserve);
    ...
    if (highContrastTheme) {
      ...
      tipOpt.palette.setBrush(QPalette::WindowText,
                              option->palette.toolTipText());
    }
    proxy()->drawPrimitive(PrimitiveElement(PE_FluentFlyoutSurface), &tipOpt,
                           painter, widget);
  ...
  case PrimitiveElement(PE_FluentFlyoutSurface): {
    ...
    drawPopupShadow(painter, panelRect, cBRoundingRadius,
                    FlyoutShadowBorderWidth);
    drawRoundedBorderSurface(
        painter, panelRect, cBRoundingRadius,
        ...
        highContrastTheme ? 2.0 : 1.0, true);
    break;
  }
  case PE_FrameTabWidget:
    ...
    if (const QStyleOptionTabWidgetFrame *frame =
      ...
      const auto rect =
          QRectF(option->rect).marginsRemoved(QMarginsF(0.5, 0.5, 0.5, 0.5));
  ...
  case PE_FrameGroupBox:
    if (const QStyleOptionFrame *frame =
      ...
      if (frame->features & QStyleOptionFrame::Flat) {
        ...
        const auto frameRect =
            QRectF(frame->rect).marginsRemoved(QMarginsF(1.5, 1.5, 1.5, 1.5));
  ...
  case PE_IndicatorBranch: {
    ...
    if (option->state & State_Children) {
      ...
      painter->setFont(f);
      painter->setPen(
          option->palette.color(isOpen ? QPalette::Active : QPalette::Disabled,
                                QPalette::WindowText));
      // 用每行唯一的 key 读取动画值，避免节点间动画状态共享
      const QByteArray animKey = QByteArrayLiteral("_q_branch_anim_") +
                                 QByteArray::number(option->rect.y());
      QNumberStyleAnimation *animation =
          widget ? qobject_cast<QNumberStyleAnimation *>(
                       getAnimationEx(const_cast<QWidget *>(widget), animKey))
                 : nullptr;
      ...
      const QPointF c =
          option->rect.center() + QPointF(isReverse ? -4.0 : 4.0, 0.0);
      painter->translate(c);
      painter->rotate(angle);
      const int half = qMin(option->rect.width(), option->rect.height()) / 2;
      QRect r(-half, -half, half * 2, half * 2);
      painter->drawText(r, Qt::AlignCenter,
                        isReverse ? ChevronLeftMed : ChevronRightMed);
      painter->restore();
    }
  } break;
  case PE_IndicatorRadioButton: {
    const bool isOn = option->state & State_On;
    int indicatorSize = proxy()->pixelMetric(QStyle::PM_ExclusiveIndicatorWidth,
                                             option, widget);
    qreal outerRadius = indicatorSize / 2.0;
    qreal innerRadius =
        radioButtonInnerRadius(state, option, widget, indicatorSize);
    if (transitionsEnabled() && option->styleObject) {
      ...
      QNumberStyleAnimation *animation = qobject_cast<QNumberStyleAnimation *>(
          getAnimation(option->styleObject));
      innerRadius =
          animation ? animation->currentValue()
  ...
  case PE_PanelButtonTool: {
    ...
    bool accent =
        widget && widget->property(ButtonAccentStyleProperty).toBool();
    if (accent) {
      painter->setBrush(calculateAccentColor(option));
    } else {
  ...
  case PE_PanelButtonBevel: {
    ...
    QRectF rect = option->rect.marginsRemoved(
        QMargins(cBShadowBorderWidth, cBShadowBorderWidth, cBShadowBorderWidth,
                 cBShadowBorderWidth));
    ...
    if (element == PE_PanelButtonTool &&
        ((!isMouseOver && !isRaised) || !isEnabled)) {
      painter->setPen(Qt::NoPen);
    } else {
      painter->setPen(winUI3Color(controlStrokePrimary));
    }
    ...
    if (shape != ButtonShapeCircle) {
      ...
      const QRectF bottomClip(rect.left(), rect.bottom() - 1.0, rect.width(),
                              1.5);
  ...
  case PE_FrameDefaultButton:
    ...
    painter->setBrush(Qt::NoBrush);
    painter->drawRoundedRect(option->rect, secondLevelRoundingRadius,
                             secondLevelRoundingRadius);
    break;
  case PE_FrameMenu:
    break;
  case PE_PanelMenu: {
    if (widget &&
        widget->property(MenuPopupSuppressPaintingProperty).toBool()) {
      break;
    }
    QStyleOption menuOpt(*option);
    menuOpt.rect = option->rect.marginsRemoved(
        QMargins(FlyoutShadowBorderWidth, FlyoutShadowBorderWidth,
                 FlyoutShadowBorderWidth, FlyoutShadowBorderWidth));
    proxy()->drawPrimitive(PrimitiveElement(PE_FluentFlyoutSurface), &menuOpt,
                           painter, widget);
    break;
  }
  case PE_PanelLineEdit:
    if (const auto *panel =
            qstyleoption_cast<const QStyleOptionFrame *>(option)) {
      const bool isInSpinBox = widget && qobject_cast<const QAbstractSpinBox *>(
                                             widget->parent()) != nullptr;
      const bool isInComboBox = widget && qobject_cast<const QComboBox *>(
                                              widget->parent()) != nullptr;
      if (!isInSpinBox && !isInComboBox) {
        const auto frameRect =
            QRectF(option->rect).marginsRemoved(QMarginsF(1.5, 1.5, 1.5, 1.5));

        auto inputFillBrush = [this](const QStyleOption *option,
                                     const QWidget *widget) -> QBrush {
          // slightly different states than in controlFillBrush
          using namespace StyleOptionHelper;
          const auto role =
              widget ? widget->backgroundRole() : QPalette::Window;
        ...
        const QColor opaqueBg =
            resolveOpaque(inputFillBrush(option, widget).color(),
                          option->palette.base().color());
        drawRoundedRect(painter, frameRect, Qt::NoPen, opaqueBg);
  ...
  case PE_Frame: {
    if (const auto *frame =
      ...
      const bool isComboPopup =
          widget && widget->inherits("QComboBoxPrivateContainer");

      painter->setPen(Qt::NoPen);

      bool noRoundedCorners =
          widget && widget->property(NoRoundedCorners).toBool();
      if (isComboPopup) {
        QStyleOption popupOpt(*option);
        popupOpt.rect =
            rect.marginsRemoved(comboBoxPopupPanelMargins(widget)).toRect();
        proxy()->drawPrimitive(PrimitiveElement(PE_FluentFlyoutSurface),
                               &popupOpt, painter, widget);
        break;
      } else {
      ...
      if (noRoundedCorners) {
        ...
        drawLineEditFrame(painter, borderRect, option,
  ...
  case PE_PanelItemViewRow:
    if (const QStyleOptionViewItem *vopt =
      ...
      bool hightlightCurrent =
          (vopt->state & (State_Selected | State_MouseOver)) != 0;
      const bool isNonTableHighlight =
          !qobject_cast<const QTableView *>(widget) && hightlightCurrent &&
          !highContrastTheme;
      const bool isNavView =
          qobject_cast<const QTreeView *>(widget) &&
          widget->property(NavigationViewStyleProperty).toBool();
      const bool isPlainTreeView =
          qobject_cast<const QTreeView *>(widget) && !isNavView;
      ...
      if (isNonTableHighlight) {
        ...
        isDecorationColumn = vopt->features.testAnyFlags(
        ...
        if (isDecorationColumn && vopt->showDecorationSelected) {
          ...
          const QColor itemHighlight = isSelected
          ...
          if (isFirst) {
            ...
            painter->drawRoundedRect(rect.marginsRemoved(QMargins(
                                         2, 2, -secondLevelRoundingRadius, 2)),
                                     secondLevelRoundingRadius,
                                     secondLevelRoundingRadius);
          } else if (isLast) {
            PainterStateGuard psg(painter);
            painter->setClipRect(rect);
            painter->drawRoundedRect(rect.marginsRemoved(QMargins(
                                         -secondLevelRoundingRadius, 2, 2, 2)),
                                     secondLevelRoundingRadius,
                                     secondLevelRoundingRadius);
          } else {
  ...
  case PE_Widget: {
    ...
    if (qobject_cast<const QMessageBox *>(widget)) {
      buttonBox = widget->findChild<const QDialogButtonBox *>(
          QLatin1String("qt_msgbox_buttonbox"));
    }
    ...
    if (widget && widget->property("isCard").toBool()) {
      ...
      if (highContrastTheme) {
        ...
        const bool wallpaperMode =
            qApp && qApp->property("_q_widget_mode").toInt() >= 1;
        brColor = winUI3CardBackgroundColor(
            option->palette, colorSchemeIndex == 1, wallpaperMode);
      }
    ...
    if (const QTabBar *tabBar = qobject_cast<const QTabBar *>(widget)) {
      const QColor backgroundColor = segmentedThemeColorProperty(
          tabBar, colorSchemeIndex == 1, SegmentedBackgroundColorProperty,
          SegmentedBackgroundColorDarkProperty,
          highContrastTheme ? option->palette.button().color()
                            : winUI3Color(fillControlAltSecondary));
      const bool segmentedSemiRound =
          segmentedBoolProperty(tabBar, SegmentedSemiRoundProperty, false);
      const QColor borderColor = highContrastTheme
      ...
      if (tabBar->count() > 0) {
        ...
        if (bgRect.isValid()) {
          const qreal radius = segmentedSemiRound ? bgRect.height() / 2.0
                                                  : secondLevelRoundingRadius;
    ...
    if (widget && widget->palette().isBrushSet(QPalette::Active,
  ...
  case PE_FrameWindow:
    if (const auto *frm =
      ...
      int fwidth = int((frm->lineWidth + frm->midLineWidth) /
                       nativeMetricScaleFactor(widget));

      QRectF bottomLeftCorner = QRectF(
          rect.left() + 1.0, rect.bottom() - 1.0 - secondLevelRoundingRadius,
          secondLevelRoundingRadius, secondLevelRoundingRadius);
      QRectF bottomRightCorner =
          QRectF(rect.right() - 1.0 - secondLevelRoundingRadius,
      ...
      if (widget != nullptr) {
        ...
        maskPainter.drawRoundedRect(option->rect, secondLevelRoundingRadius,
                                    secondLevelRoundingRadius);
      ...
      painter->drawLine(
          ...
          QPointF(rect.left() + 0.5,
                  rect.bottom() - 0.5 - secondLevelRoundingRadius));
      painter->drawLine(QPointF(rect.left() + 0.5 + secondLevelRoundingRadius,
                                rect.bottom() - 0.5),
                        QPointF(rect.right() - 0.5 - secondLevelRoundingRadius,
                                rect.bottom() - 0.5));
      painter->drawLine(
          QPointF(rect.right() - 0.5, rect.top() + 1.5),
          QPointF(rect.right() - 0.5,
                  rect.bottom() - 0.5 - secondLevelRoundingRadius));

      painter->setPen(Qt::NoPen);
      painter->setBrush(frm->palette.base());
      painter->drawPie(
          bottomRightCorner.marginsAdded(QMarginsF(2.5, 2.5, 0.0, 0.0)),
          270 * 16, 90 * 16);
      painter->drawPie(
          bottomLeftCorner.marginsAdded(QMarginsF(0.0, 2.5, 2.5, 0.0)),
          -90 * 16, -90 * 16);
  ...
  case PE_IndicatorToolBarSeparator: {
    ...
    if (option->state & State_Horizontal) {
      ...
      painter->drawLine(QPoint(x1, option->rect.top() + margin),
                        QPoint(x1, option->rect.bottom() - margin));
    } else {
      int y1 = option->rect.center().y();
      painter->drawLine(QPoint(option->rect.left() + margin, y1),
                        QPoint(option->rect.right() - margin, y1));
    }
    painter->setPen(pen);
    return;
  }
...
QRect FluentUI3Style::subElementRect(SubElement element,
                                     const QStyleOption *option,
                                     const QWidget *widget) const {
  QRect ret;
  switch (element) {
  ...
  case SE_ProgressBarLabel:
    if (const auto *pb =
      ...
      if (widget && widget->property(ProgressBarStyleProperty).toInt() !=
                        ProgressBarRing) {
      ...
      QRect progressRect =
          QCommonStyle::subElementRect(element, &optCopy, widget);
      const int styleValue =
          widget ? widget->property(ProgressBarStyleProperty).toInt()
                 : ProgressBarThin;
      ...
      const bool hasVisibleLabel =
          pb->textVisible && (pb->state & State_Horizontal);
      if (element != SE_ProgressBarLabel && styleValue != ProgressBarRing &&
          !hasVisibleLabel) {
        ...
        if (pb->state & QStyle::State_Horizontal) {
          progressRect.setTop(progressRect.top() +
                              (progressRect.height() - thickness) / 2);
          progressRect.setHeight(thickness);
        } else {
          progressRect.setLeft(progressRect.left() +
                               (progressRect.width() - thickness) / 2);
...
QRect FluentUI3Style::subControlRect(ComplexControl control,
  ...
  case CC_SpinBox:
    if (const QStyleOptionSpinBox *spinbox =
      ...
      const int fw = spinbox->frame
                         ? pixelMetric(PM_SpinBoxFrameWidth, spinbox, widget)
                         : 0;
      const int buttonHeight = hasButtons
                                   ? qMin(spinbox->rect.height() - 3 * fw,
                                          spinbox->fontMetrics.height() * 5 / 4)
                                   : 0;
      const QSize buttonSize(buttonHeight * 6 / 5, buttonHeight);
      const int textFieldLength =
          spinbox->rect.width() - 2 * fw - 2 * buttonSize.width();
      ...
      if (!horizonal) {
        ...
        if (subControl == SC_SpinBoxUp) {
          return QRect(QPoint(r.topRight().x() - buttonSize.width(),
                              r.topRight().y() + fw * 2),
                       QSize(buttonSize.width(), btnHeight));
        }
        if (subControl == SC_SpinBoxDown) {
          return QRect(QPoint(r.topRight().x() - buttonSize.width(),
                              r.bottomRight().y() - btnHeight - fw),
                       QSize(buttonSize.width(), btnHeight));
        }
      ...
      case SC_SpinBoxDown: {
        ...
        if (buttonLayout == SpinBoxButtonLayout::ArrowsHorizontalRight) {
          ret = QRect(r.x() + textFieldLength, r.y() + yOfs, buttonSize.width(),
                      buttonSize.height());
          if (subControl == SC_SpinBoxUp) {
        ...
        {
          ret = QRect(r.x() + 1, r.y() + (r.height() - buttonSize.height()) / 2,
                      buttonSize.width(), buttonSize.height());
          if (subControl == SC_SpinBoxUp) {
            ret = QRect(ret.right() + textFieldLength - 1, ret.y(),
                        buttonSize.width(), buttonSize.height());
          }
          break;
        }
      ...
      case SC_SpinBoxEditField:
        if (buttonLayout == SpinBoxButtonLayout::ArrowsHorizontalRight) {
          ret = QRect(r.x(), r.y(), r.width() - 2 * buttonSize.width(),
                      r.height());
        } else {
          ret = QRect(r.x() + buttonSize.width(), r.y(),
                      r.width() - 2 * buttonSize.width(), r.height());
        }
        break;
  ...
  case CC_TitleBar:
    if (const QStyleOptionTitleBar *titlebar =
      ...
      const int iconSize = proxy()->pixelMetric(
          QStyle::PM_TitleBarButtonIconSize, option, widget);
      ...
      case SC_TitleBarLabel:
        if (titlebar->titleBarFlags &
            (Qt::WindowTitleHint | Qt::WindowSystemMenuHint)) {
          ret = titlebar->rect;
          if (titlebar->titleBarFlags & Qt::WindowSystemMenuHint) {
            ret.adjust(iconSize + controlWidthMargin + indent, 0, -controlWidth,
                       0);
          }
      ...
      case SC_TitleBarSysMenu:
        if (titlebar->titleBarFlags & Qt::WindowSystemMenuHint) {
          const auto yOfs =
              titlebar->rect.top() + (titlebar->rect.height() - iconSize) / 2;
          ret.setRect(titlebar->rect.left() + controlWidthMargin + indent, yOfs,
                      iconSize, iconSize);
        }
        break;
      default:
      ...
      if (widget && isMinimized && titlebar->rect.width() < offset) {
        const_cast<QWidget *>(widget)->resize(controlWidthMargin + indent +
                                                  offset + iconSize +
                                                  controlWidthMargin,
                                              controlWidth);
      }
  ...
  case CC_ComboBox: {
    if (const auto *cb =
      ...
      case SC_ComboBoxArrow: {
        const int fw =
            cb->frame ? proxy()->pixelMetric(PM_ComboBoxFrameWidth, cb, widget)
                      : 0;
        const int buttonHeight =
            qMin(cb->rect.height() - 3 * fw, cb->fontMetrics.height() * 5 / 4);
        const QSize buttonSize(buttonHeight * 6 / 5, buttonHeight);
        const int textFieldLength =
            cb->rect.width() - 2 * fw - buttonSize.width();
        const QPoint topLeft(cb->rect.topLeft() + QPoint(fw, fw));
        const int yOfs =
            ((cb->rect.height() - 2 * fw) - buttonSize.height()) / 2;
        ret = QRect(topLeft.x() + textFieldLength, topLeft.y() + yOfs,
                    buttonSize.width(), buttonSize.height());
      ...
      case SC_ComboBoxEditField: {
        ...
        if (cb->frame) {
          const int fw =
              proxy()->pixelMetric(PM_ComboBoxFrameWidth, cb, widget);
      ...
      case SC_ComboBoxListBoxPopup: {
        ...
        if (option->direction == Qt::RightToLeft) {
          const int windowRight =
              outer.right() - inset + shadow + (panelHorizontalExpansion - 1);
          ret.setRect(windowRight - windowWidth + 1, ret.top(), windowWidth,
                      ret.height());
        } else {
          ret.setRect(outer.left() + xOffset, ret.top(), windowWidth,
                      ret.height());
        }
        break;
      }
...
void FluentUI3Style::drawTabBarTabShape(const QStyleOption *option,
                                        QPainter *painter,
                                        const QWidget *widget) const {
  if (const QStyleOptionTab *tab =
          qstyleoption_cast<const QStyleOptionTab *>(option)) {
    TabBarStyle tabBarStyle =
        (widget && widget->property(TabBarStyleProperty).isValid())
...
void FluentUI3Style::drawCapsuleTab(const QStyleOptionTab *tab,
                                    QPainter *painter,
                                    const QWidget *widget) const {
  Q_UNUSED(widget)
#if QT_CONFIG(tabbar)
  ...
  if (tab->state & State_Selected) {
    ...
    QList<QPointF> pts;
    pts << QPointF(tabRect.bottomLeft().x() - arcLength,
                   tabRect.bottomLeft().y())
        << tabRect.bottomLeft() << tabRect.topLeft() << tabRect.topRight()
        << tabRect.bottomRight()
        << QPointF(tabRect.bottomRight().x() + arcLength,
                   tabRect.bottomRight().y());
    ...
    QList<QPointF> pts;
    pts << tabRect.bottomLeft() << tabRect.topLeft() << tabRect.topRight()
        << tabRect.bottomRight();
  ...
  painter->setBrush(Qt::NoBrush);
  painter->setPen(highContrastTheme == true
                      ? tab->palette.buttonText().color()
                      : WINUI3Colors[colorSchemeIndex][frameColorLight]);
#endif // QT_CONFIG(tabbar)
}

void FluentUI3Style::drawPivotGrowingTab(const QStyleOptionTab *tab,
                                         QPainter *painter,
                                         const QWidget *widget) const {
#if QT_CONFIG(tabbar)
  ...
  const qreal contentLeft =
      r.left() + indicatorMargin + (innerWidth - targetWidth) / 2.0;
  ...
  const int previousTabIndex =
      styleObject->property("_q_pivot_grow_selected_tab_index").toInt();
  ...
  bool selectionChanged = (currentTabIndex >= 0 && previousTabIndex >= 0 &&
  ...
  const QRect indicator(
      qRound(contentLeft + (targetWidth - currentWidth) / 2.0),
      qRound(r.bottom() - indicatorHeight), qRound(currentWidth),
  ...
  painter->drawRoundedRect(indicator, indicatorHeight / 2.0,
                           indicatorHeight / 2.0);
#else
  Q_UNUSED(tab)
  Q_UNUSED(painter)
...
void FluentUI3Style::drawPivotStretchingTab(const QStyleOptionTab *tab,
                                            QPainter *painter,
                                            const QWidget *widget) const {
#if QT_CONFIG(tabbar)
  if (!tab || !tab->state.testFlag(QStyle::State_Selected)) {
  ...
  const qreal targetLeft =
      r.left() + indicatorMargin + (innerWidth - targetWidth) / 2.0;
  const qreal targetTop = r.bottom() - indicatorHeight;
  const qreal targetW = targetWidth;
  const qreal targetH = indicatorHeight;
  const auto indicatorRect = [&](qreal startLeft, qreal startTop,
                                 qreal startWidth, qreal startHeight,
                                 qreal endLeft, qreal endTop, qreal endWidth,
                                 qreal endHeight, qreal progressValue) {
    ...
    return QRectF(drawLeft, lerp(startTop, endTop, smoothStep(p)),
  ...
  const int previousTabIndex =
      styleObject->property("_q_pivot_selected_tab_index").toInt();
  bool isMovingTab = false;
#if QT_VERSION >= QT_VERSION_CHECK(6, 6, 0)
  isMovingTab = tab->position == QStyleOptionTab::TabPosition::Moving;
#endif
  ...
  qreal fromLeft =
      styleObject->property("_q_pivot_indicator_from_left").toReal();
  ...
  qreal fromH =
      styleObject->property("_q_pivot_indicator_from_height").toReal();
  ...
  const QRectF indicatorRectF = indicatorRect(
      fromLeft, fromTop, fromW, fromH, toLeft, toTop, toW, toH, progress);
  ...
  QRect indicator(qRound(indicatorRectF.left()), qRound(indicatorRectF.top()),
...
void FluentUI3Style::drawPivotSlidingTab(const QStyleOptionTab *tab,
                                         QPainter *painter,
                                         const QWidget *widget) const {
#if QT_CONFIG(tabbar)
  if (!tab || !tab->state.testFlag(QStyle::State_Selected)) {
  ...
  const int previousTabIndex =
      styleObject->property("_q_pivot_slide_selected_tab_index").toInt();
  bool isMovingTab = false;
#if QT_VERSION >= QT_VERSION_CHECK(6, 6, 0)
  isMovingTab = tab->position == QStyleOptionTab::TabPosition::Moving;
#endif
  ...
  const QRectF indicatorRectF(drawLeft,
                              lerp(fromTop, toTop, smoothStep(progress)), barW,
                              indicatorHeight);

  styleObject->setProperty("_q_pivot_slide_selected_tab_index",
                           currentTabIndex);
  ...
  painter->setPen(Qt::NoPen);
  painter->setBrush(calculateAccentColor(tab));
  painter->drawRoundedRect(indicatorRectF, indicatorHeight / 2.0,
                           indicatorHeight / 2.0);
#else
  Q_UNUSED(tab)
  Q_UNUSED(painter)
...
void FluentUI3Style::drawPillTab(const QStyleOptionTab *tab, QPainter *painter,
                                 const QWidget *widget) const {
  Q_UNUSED(widget)
#if QT_CONFIG(tabbar)
  ...
  const bool isFirstSegment = tab->position == QStyleOptionTab::Beginning ||
                              tab->position == QStyleOptionTab::OnlyOneTab;
  const bool isLastSegment = tab->position == QStyleOptionTab::End ||
                             tab->position == QStyleOptionTab::OnlyOneTab;

  const QRectF tabRect = tab->rect.marginsRemoved(
      QMargins(isFirstSegment ? 1 : 2, 2, isLastSegment ? 1 : 2, 2));
  ...
  QColor fillColor =
      isSelected ? selectedColor
                 : (isHover ? hoverColor : tab->palette.window().color());
...
void FluentUI3Style::drawSegmentedSlideTab(const QStyleOptionTab *tab,
                                           QPainter *painter,
                                           const QWidget *widget) const {
#if QT_CONFIG(tabbar)
  if (!tab || !painter) {
  ...
  const auto lerpRect = [&](const QRectF &from, const QRectF &to, qreal t) {
    return QRectF(
        lerp(from.left(), to.left(), t), lerp(from.top(), to.top(), t),
        lerp(from.width(), to.width(), t), lerp(from.height(), to.height(), t));
  };

  const bool isSelected = tab->state.testFlag(QStyle::State_Selected);
  const bool isPressed =
      tab->state.testFlag(QStyle::State_Sunken) && !isSelected;
  const bool isHover =
      tab->state.testFlag(QStyle::State_MouseOver) && !isSelected && !isPressed;
  const bool isFirstSegment = tab->position == QStyleOptionTab::Beginning ||
                              tab->position == QStyleOptionTab::OnlyOneTab;
  const bool isLastSegment = tab->position == QStyleOptionTab::End ||
                             tab->position == QStyleOptionTab::OnlyOneTab;
  ...
  const bool segmentedSemiRound =
      segmentedBoolProperty(widget, SegmentedSemiRoundProperty, false);
  const QColor hoverColor = segmentedThemeColorProperty(
      widget, colorSchemeIndex == 1, SegmentedHoverColorProperty,
      SegmentedHoverColorDarkProperty, winUI3Color(subtleHighlightColor));
  const QColor pressedColor = segmentedThemeColorProperty(
      widget, colorSchemeIndex == 1, SegmentedPressedColorProperty,
      SegmentedPressedColorDarkProperty, winUI3Color(subtlePressedColor));
  const QColor selectedColor = segmentedThemeColorProperty(
      widget, colorSchemeIndex == 1, SegmentedSelectedColorProperty,
      SegmentedSelectedColorDarkProperty,
      highContrastTheme ? tab->palette.highlight().color()
                        : winUI3Color(tabBarSelectedBackground));
  ...
  const int leftInset =
      isFirstSegment ? selectionOuterMarginH : selectionInnerSpacingH / 2;
  const int rightInset =
      isLastSegment ? selectionOuterMarginH : selectionInnerSpacingH / 2;
  const QRectF adjustedTabRect = QRectF(tab->rect).adjusted(
      leftInset, selectionMarginV, -rightInset, -selectionMarginV);

  painter->setPen(Qt::NoPen);
  if (isPressed) {
    painter->setBrush(pressedColor);
    const qreal tabRadius = segmentedSemiRound ? adjustedTabRect.height() / 2.0
                                               : secondLevelRoundingRadius;
  ...
  const int previousTabIndex =
      styleObject->property("_q_segmented_selected_tab_index").toInt();
  ...
  const int targetLeftInset =
      targetIsFirst ? selectionOuterMarginH : selectionInnerSpacingH / 2;
  const int targetRightInset =
      targetIsLast ? selectionOuterMarginH : selectionInnerSpacingH / 2;
  const QRect targetRect = tabBar->tabRect(currentTabIndex)
                               .adjusted(targetLeftInset, selectionMarginV,
                                         -targetRightInset, -selectionMarginV);
  QRect previousRect =
      styleObject->property("_q_segmented_previous_selected_rect").toRect();
  QRect currentRect =
      styleObject->property("_q_segmented_current_selected_rect").toRect();
  ...
  const qreal progress = clamp01(animationValue(styleObject, animKey, 1.0f));
  QRectF drawRect =
      lerpRect(QRectF(previousRect), QRectF(currentRect), progress);
  ...
  const qreal selectedRadius =
      segmentedSemiRound ? drawRect.height() / 2.0 : secondLevelRoundingRadius;
...
void FluentUI3Style::drawSegmentedFadeTab(const QStyleOptionTab *tab,
                                          QPainter *painter,
                                          const QWidget *widget) const {
#if QT_CONFIG(tabbar)
  if (!tab || !painter) {
  ...
  int selectedTabIndex =
      styleObject->property("_q_segmented_fade_selected_tab_index").toInt();
  int fadingFromIndex =
      styleObject->property("_q_segmented_fade_from_index").toInt();
  const QByteArray animKey = "_q_segmented_fade_fill";

  if (isSelected && currentTabIndex >= 0 && selectedTabIndex < 0) {
    selectedTabIndex = currentTabIndex;
    fadingFromIndex = -1;
  } else if (isSelected && currentTabIndex >= 0 && selectedTabIndex >= 0 &&
  ...
  if (!shouldDrawSelectedFill) {
    ...
    styleObject->setProperty("_q_segmented_fade_selected_tab_index",
                             selectedTabIndex);
    return;
  }
  ...
  painter->setBrush(selectedBrush);
  const qreal selectedRadius = segmentedSemiRound
                                   ? adjustedTabRect.height() / 2.0
                                   : secondLevelRoundingRadius;
...
void FluentUI3Style::drawSegmentedWinUI3Tab(const QStyleOptionTab *tab,
                                            QPainter *painter,
                                            const QWidget *widget) const {
#if QT_CONFIG(tabbar)
  if (!tab || !painter) {
  ...
  const int currentTabIndex = tabBar->currentIndex();
  QVariant activeTabIndexVar =
      styleObject->property("_q_segmented_winui3_curr_sel");
  int activeTabIndex =
      activeTabIndexVar.isValid() ? activeTabIndexVar.toInt() : -1;

  QVariant previousTabIndexVar =
      styleObject->property("_q_segmented_winui3_prev_sel");
  int previousTabIndex =
      previousTabIndexVar.isValid() ? previousTabIndexVar.toInt() : -1;
  ...
  QVariant currentPressedIndexVar =
      styleObject->property("_q_segmented_winui3_pressed_index");
  int currentPressedIndex =
      currentPressedIndexVar.isValid() ? currentPressedIndexVar.toInt() : -1;

  QVariant activePressedIndexVar =
      styleObject->property("_q_segmented_winui3_curr_press");
  int activePressedIndex =
      activePressedIndexVar.isValid() ? activePressedIndexVar.toInt() : -1;

  QVariant previousPressedIndexVar =
      styleObject->property("_q_segmented_winui3_prev_press");
  int previousPressedIndex =
      previousPressedIndexVar.isValid() ? previousPressedIndexVar.toInt() : -1;

  if (currentPressedIndex != activePressedIndex) {
    previousPressedIndex = activePressedIndex;
    activePressedIndex = currentPressedIndex;
    styleObject->setProperty("_q_segmented_winui3_prev_press",
                             previousPressedIndex);
    styleObject->setProperty("_q_segmented_winui3_curr_press",
                             activePressedIndex);
  ...
  qreal selAnimProgress = clamp01(
      animationValue(styleObject, "_q_segmented_winui3_sel_anim", 1.0f));
  qreal pressAnimProgress = clamp01(
      animationValue(styleObject, "_q_segmented_winui3_press_anim", 1.0f));
  ...
  const int rightInset =
      isLastSegment ? selectionOuterMarginH : selectionInnerSpacingH / 2;
  QRectF adjustedTabRect = QRectF(tab->rect).adjusted(
      leftInset, selectionMarginV, -rightInset, -selectionMarginV);

  if (!isSelected) {
    if (isHover || isPressedReal) {
      painter->setPen(Qt::NoPen);
      painter->setBrush(isPressedReal ? winUI3Color(subtlePressedColor)
                                      : winUI3Color(subtleHighlightColor));

      QRectF hoverRect = adjustedTabRect.marginsRemoved(QMarginsF(3, 3, 3, 3));
      painter->drawRoundedRect(hoverRect, secondLevelRoundingRadius,
                               secondLevelRoundingRadius);
    }
  }
  ...
  if (selectProgress > 0.0 || isSelected) {
    if (isSelected) {
      const QColor selectedBrush = highContrastTheme
                                       ? tab->palette.highlight().color()
                                       : winUI3Color(fillControlDefault);
      painter->setBrush(selectedBrush);
      painter->setPen(QPen(highContrastTheme ? tab->palette.buttonText().color()
                                             : winUI3Color(frameColorLight),
                           1));

      QRectF drawRect = adjustedTabRect;
      drawRect.adjust(0.5, 0.5, -0.5, -0.5);
      painter->drawRoundedRect(drawRect, secondLevelRoundingRadius,
                               secondLevelRoundingRadius);
    }

    qreal targetIndicatorWidth = 16.0;
    qreal currentIndicatorWidth =
        targetIndicatorWidth * selectProgress * (1.0 - 0.5 * pressProgress);

    if (currentIndicatorWidth > 0.5) {
      QRectF indicatorRect =
          QRectF(adjustedTabRect.x() + (adjustedTabRect.width() / 2.0 -
                                        currentIndicatorWidth / 2.0),
                 adjustedTabRect.bottom() - 3.5, currentIndicatorWidth, 2);
...
void FluentUI3Style::drawNavigationTab(const QStyleOptionTab *tab,
                                       QPainter *painter,
                                       const QWidget *widget) const {
#if QT_CONFIG(tabbar)
  if (!tab || !painter) {
  ...
  const bool isSelected = tab->state.testFlag(QStyle::State_Selected);
  const bool isHover =
      tab->state.testFlag(QStyle::State_MouseOver) && !isSelected;
  const bool isWest = tab->shape == QTabBar::RoundedWest ||
                      tab->shape == QTabBar::TriangularWest;
  const bool isEast = tab->shape == QTabBar::RoundedEast ||
                      tab->shape == QTabBar::TriangularEast;
  const QTabBar *tabBar = qobject_cast<const QTabBar *>(widget);
  const bool isFirstTab = tab->position == QStyleOptionTab::Beginning ||
                          tab->position == QStyleOptionTab::OnlyOneTab;
  const bool isLastTab = tab->position == QStyleOptionTab::End ||
                         tab->position == QStyleOptionTab::OnlyOneTab;
  ...
  const int topInset =
      isFirstTab ? navigationOuterMarginV : navigationSpacingV / 2;
  const int bottomInset =
      isLastTab ? navigationOuterMarginV : navigationSpacingV / 2;
  const QRectF itemRect = QRectF(tab->rect).adjusted(
      navigationInsetH, topInset, -navigationInsetH, -bottomInset);
  if (isSelected) {
    painter->setBrush(winUI3Color(tabBarSelectedBackground));
    painter->drawRoundedRect(itemRect, secondLevelRoundingRadius,
                             secondLevelRoundingRadius);
  } else if (isHover) {
    painter->setBrush(winUI3Color(tabBarHoverBackground));
    painter->drawRoundedRect(itemRect, secondLevelRoundingRadius,
  ...
  const qreal indicatorX =
      isEast ? itemRect.right() - indicatorWidth : itemRect.left();
  ...
  if (tabBar && tab->styleObject) {
    ...
    const int previousTabIndex =
        tab->styleObject->property("_q_navigation_selected_tab_index").toInt();
    const QByteArray animKey = "_q_tab_navigation_indicator_grow";

    if (currentTabIndex >= 0 && previousTabIndex >= 0 &&
        currentTabIndex != previousTabIndex && tabIndex == currentTabIndex) {
    ...
    const qreal progress =
        previousTabIndex < 0
            ? 1.0
            : qBound(0.0,
                     qreal(animationValue(tab->styleObject, animKey, 1.0f)),
                     1.0);
    const qreal fullHeight = indicatorHeight;
    indicatorHeight = fullHeight * progress;
    indicatorTop =
        itemRect.top() + indicatorInset + (fullHeight - indicatorHeight) / 2.0;

    tab->styleObject->setProperty("_q_navigation_selected_tab_index",
                                  currentTabIndex);
  }

  const QRectF indicatorRect(indicatorX, indicatorTop, indicatorWidth,
                             indicatorHeight);

  const QColor accent = calculateAccentColor(tab);
  painter->setBrush(accent);
  painter->drawRoundedRect(indicatorRect, indicatorWidth / 2.0,
                           indicatorWidth / 2.0);
#else
  Q_UNUSED(tab)
  Q_UNUSED(painter)
...
void FluentUI3Style::drawListViewIndicator(const QStyleOptionViewItem *option,
                                           QPainter *painter,
                                           const QWidget *widget) const {
  ...
  QObject *stateObject = lv->viewport() ? static_cast<QObject *>(lv->viewport())
                                        : const_cast<QListView *>(lv);
  const QByteArray animKey = "_q_list_indicator_grow";
  const int currentRow = option->index.row();
  const int storedRow =
      stateObject->property("_q_list_indicator_selected_row").toInt();
  ...
  const bool isRtl = option->direction == Qt::RightToLeft;
  const qreal xPos =
      isRtl ? option->rect.right() - 4.5f : option->rect.left() + 3.5f;
  const qreal halfHeight =
      (option->rect.height() - normalInset * 2.0) * progress * 0.5;
  ...
  painter->setBrush(col);
  painter->setPen(col);
  painter->drawRoundedRect(indicatorRect, selectionIndicatorRadius,
                           selectionIndicatorRadius);
}

void FluentUI3Style::drawNavigationViewIndicator(
  ...
  QObject *stateObject = treeView->viewport()
                             ? static_cast<QObject *>(treeView->viewport())
                             : const_cast<QTreeView *>(treeView);
  ...
  const QRectF fromRect =
      stateObject->property("_q_tree_indicator_from_rect").toRectF();
  const QRectF toRect =
      stateObject->property("_q_tree_indicator_to_rect").toRectF();
  ...
  QModelIndex fromIndex =
      stateObject->property("_q_tree_indicator_from_index").toModelIndex();
  QModelIndex toIndex =
      stateObject->property("_q_tree_indicator_to_index").toModelIndex();
  ...
  const bool hasSelection =
      treeView->selectionModel() && treeView->selectionModel()->hasSelection();
  ...
  if (toH <= 0.0 && hasSelection && phase.isEmpty() &&
      treeView->property("navigationDirection").toString().isEmpty()) {
  ...
  if (!inAnimation && phase.isEmpty()) {
    ...
    if (wasSelected && !hasSelection && toIndex.isValid() && toH > 0.0) {
      ...
      fromH = toH;
      fromIndex = toIndex;
      toTop = (navDir == QStringLiteral("up"))
                  ? (fromTop - normalInset)
    ...
    } else if (!wasSelected && hasSelection &&
      ...
      fromX = toX;
      fromTop = (navDir == QStringLiteral("up")) ? qreal(rect.top())
                                                 : qreal(rect.bottom());
      fromH = 0.0;
      fromIndex = toIndex;
      ...
      stateObject->setProperty("_q_nav_had_selection", true);
               ...
               (option->state & State_Selected) && option->index != toIndex) {
      fromX = toX;
      fromTop = toTop;
      fromH = toH;
      toX = targetX;
  ...
  stateObject->setProperty("_q_tree_indicator_from_rect",
                           QRectF(fromX, fromTop, targetW, fromH));
  stateObject->setProperty("_q_tree_indicator_to_rect",
                           QRectF(toX, toTop, targetW, toH));
  ...
  const QRectF mark(QPointF(drawX, drawTop),
                    QPointF(drawX + targetW, drawBottom));
  painter->setBrush(calculateAccentColor(option));
  painter->setPen(calculateAccentColor(option));
  painter->drawRoundedRect(mark, selectionIndicatorRadius,
                           selectionIndicatorRadius);
}

void FluentUI3Style::drawTreeViewIndicator(const QStyleOptionViewItem *option,
                                           QPainter *painter,
                                           const QWidget *widget) const {
  if (!option || !painter || highContrastTheme) {
    return;
  ...
  if (option->state & State_Selected) {
    ...
    const QRectF mark(QPointF(xPos, rect.y() + yOfs),
                      QPointF(xPos + selectionIndicatorWidth,
                              rect.y() + rect.height() - yOfs));
...
void FluentUI3Style::drawProgressRing(const QStyleOptionProgressBar *option,
  ...
  qreal thickness =
      positiveRealProperty(ProgressBarThicknessProperty, defaultThickness);
  ...
  ringRect = ringRect.marginsRemoved(
      QMarginsF(outerInset, outerInset, outerInset, outerInset));
  if (ringRect.width() <= 0.0 || ringRect.height() <= 0.0) {
    return;
  }

  if (drawTrack) {
    QPen pen(option->palette.color(QPalette::Mid), thickness, Qt::SolidLine,
             Qt::SquareCap);
    painter->setPen(pen);
    painter->setBrush(Qt::NoBrush);
  ...
  if (isIndeterminate) {
    ...
    const int loopDurationMSec =
        widget->property(ProgressBarRingIndeterminateDurationProperty).toInt();
    ...
    const auto fillPercentage = (float(option->progress - option->minimum)) /
                                (float(option->maximum - option->minimum));
  ...
  painter->setPen(pen);
  painter->setBrush(Qt::NoBrush);
  painter->drawArc(ringRect, int((startAngle + angleZeroOffset) * 16),
                   int(spanAngle * 16));
}

// From QCommonStyle::drawControl, but only for CE_TabBarTabLabel, and with some
// adjustments for FluentUI3
void FluentUI3Style::drawTabBarTabLabel(const QStyleOption *option,
                                        QPainter *painter,
                                        const QWidget *widget) const {
  if (const QStyleOptionTab *tab =
          qstyleoption_cast<const QStyleOptionTab *>(option)) {
    const bool isVerticalNavigation =
        widget &&
        widget->property(TabBarStyleProperty).toInt() ==
    ...
    const bool isSegmentedWinUI3 =
        widget && widget->property(TabBarStyleProperty).toInt() ==
                      TabBarStyle::Segmented_WinUI3;
    if (isSegmentedWinUI3) {
      ...
      if (!tab->icon.isNull()) {
        iconRect =
            QRect(startX, tab->rect.center().y() - tab->iconSize.height() / 2,
                  iconWidth, tab->iconSize.height());
        tr = QRect(iconRect.right() + spacing, tab->rect.top(), textWidth,
                   tab->rect.height());
      } else {
      ...
      iconRect = QRect(tab->rect.center().x() - iconW / 2,
                       tab->rect.center().y() - iconH / 2, iconW, iconH);
      tr = tab->rect;
      alignment = Qt::AlignCenter | Qt::TextShowMnemonic;
    }
    ...
    if (!tab->icon.isNull()) {
      ...
      QPixmap tabIcon = tab->icon.pixmap(
          tab->iconSize, QStyleHelper::getDpr(painter),
    ...
    QColor tabTextColor;
    const bool isSegmentedCustomColors =
        widget &&
        (widget->property(TabBarStyleProperty).toInt() ==
             TabBarStyle::Segmented_Slide ||
         widget->property(TabBarStyleProperty).toInt() ==
             TabBarStyle::Segmented_Fade) &&
        widget->property(SegmentedBackgroundColorProperty).isValid();
    if (!isEnabled) {
      tabTextColor = highContrastTheme ? tab->palette.buttonText().color()
                                       : winUI3Color(textDisabled);
    } else if (isHot) {
      tabTextColor = highContrastTheme ? tab->palette.buttonText().color()
                                       : winUI3Color(textPrimary);
    } else {
      tabTextColor = highContrastTheme ? tab->palette.buttonText().color()
                                       : (isSegmentedCustomColors
                                              ? winUI3Color(textPrimary)
                                              : winUI3Color(textSecondary));
    }
    ...
    proxy()->drawItemText(painter, tr, alignment, textPalette, isEnabled,
...
void FluentUI3Style::drawControl(ControlElement element,
  ...
  case CE_ToolButtonLabel:
    ...
    if (const QStyleOptionToolButton *toolbutton =
      ...
      if (toolbutton->state & (State_Sunken | State_On)) {
        shiftX =
            proxy()->pixelMetric(PM_ButtonShiftHorizontal, toolbutton, widget);
        shiftY =
      ...
      if (((!hasArrow && toolbutton->icon.isNull()) &&
        ...
        if (widget &&
            widget->objectName() == QLatin1String("win_caption_close") &&
            (widget->underMouse() || (toolbutton->state & State_Sunken))) {
          painter->setPen((toolbutton->state & State_Sunken)
          ...
          if (accent || (checkable && checked)) {
            QColor penCol = option->state.testFlag(QStyle::State_Enabled)
                                ? winUI3Color(textOnAccentPrimary)
        ...
        if (!toolbutton->icon.isNull()) {
          ...
          if (!(toolbutton->state & State_Enabled)) {
            mode = QIcon::Disabled;
                     ...
                     (toolbutton->state & State_AutoRaise)) {
            mode = QIcon::Active;
          } else {
            mode = QIcon::Normal;
          }
          ...
          if (widget && QString(widget->metaObject()->className()) ==
                            "QDockWidgetTitleButton") {
            tBtnIconSize = QSize(15, 15);
          }
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
          pm = toolbutton->icon.pixmap(
              toolbutton->rect.size().boundedTo(tBtnIconSize),
          ...
          pm = toolbutton->icon.pixmap(
              qt_getWindow(widget),
              toolbutton->rect.size().boundedTo(tBtnIconSize), mode, state);
#endif
        ...
        if (toolbutton->toolButtonStyle != Qt::ToolButtonIconOnly) {
          ...
          if (toolbutton->toolButtonStyle == Qt::ToolButtonTextUnderIcon) {
            ...
            if (!hasArrow) {
              proxy()->drawItemPixmap(
                  painter, QStyle::visualRect(toolbutton->direction, rect, pr),
                  Qt::AlignCenter, pm);
            } else {
              drawArrow(proxy(), toolbutton, pr, painter, widget);
            }
          ...
          proxy()->drawItemText(
              painter, QStyle::visualRect(toolbutton->direction, rect, tr),
              alignment, toolbutton->palette, toolbutton->state & State_Enabled,
              text);
        } else {
          rect.translate(shiftX, shiftY);
          if (hasArrow) {
  ...
  case CE_ShapedFrame:
    if (const QStyleOptionFrame *f =
      ...
      if (isComboPopup) {
        QStyleOption popupOpt(*option);
        popupOpt.rect = QRectF(option->rect)
                            .marginsRemoved(comboBoxPopupPanelMargins(widget))
                            .toRect();
        proxy()->drawPrimitive(PrimitiveElement(PE_FluentFlyoutSurface),
                               &popupOpt, painter, widget);
        break;
      ...
      case QFrame::Box:
        if (frameShadow == QFrame::Plain) {
          qDrawPlainRoundedRect(
              painter, f->rect, secondLevelRoundingRadius,
              secondLevelRoundingRadius,
          ...
          qDrawShadeRect(painter, f->rect, f->palette,
                         frameShadow == QFrame::Sunken, lw, mlw);
        }
        break;
      case QFrame::Panel:
        if (frameShadow == QFrame::Plain) {
          ...
          qDrawShadePanel(painter, f->rect, f->palette,
                          frameShadow == QFrame::Sunken, lw);
        }
        break;
      default:
  ...
  case CE_ProgressBarContents:
    if (const auto baropt =
      ...
      QRectF rect = option->rect;
      const auto orientation = (baropt->state & QStyle::State_Horizontal)
                                   ? Qt::Horizontal
                                   : Qt::Vertical;
      ...
      if (isIndeterminate) {
        ...
        const auto elapsedTime =
            std::chrono::time_point_cast<std::chrono::milliseconds>(
                std::chrono::system_clock::now());
        const auto elapsed = elapsedTime.time_since_epoch().count();
        const auto handleCenter =
            (elapsed % loopDurationMSec) / float(loopDurationMSec);
        const auto isLongHandle = (elapsed / loopDurationMSec) % 2 == 0;
        const auto lengthFactor = (isLongHandle ? 33.0f : 25.0f) / 100.0f;
        const auto begin =
            qMax(handleCenter * (1 + lengthFactor) - lengthFactor, 0.0f);
      ...
      painter->drawRoundedRect(rect, progressBarHalfThickness,
                               progressBarHalfThickness);
    }
    break;
  case CE_ProgressBarLabel:
    if (const auto baropt =
      ...
      if (!vertical) {
        proxy()->drawItemText(painter, baropt->rect,
                              Qt::AlignCenter | Qt::TextSingleLine,
                              baropt->palette, baropt->state & State_Enabled,
                              baropt->text, QPalette::Text);
      }
    }
    break;
#endif // QT_CONFIG(progressbar)
  ...
  case CE_PushButtonLabel:
    if (const QStyleOptionButton *btn =
      ...
      QRect textRect = btn->rect.marginsRemoved(
          QMargins(contentHMargin, 0, contentHMargin, 0));
      ...
      if (!btn->icon.isNull()) {
        ...
        QRect iconRect = QRect(textRect.x(), textRect.y(),
                               btn->iconSize.width(), textRect.height());
      ...
      proxy()->drawItemText(painter, vTextRect, tf, option->palette, isEnabled,
                            btn->text);
    }
    break;
    ...
    if (const QStyleOptionButton *btn =
      ...
      if (btn->features.testFlag(QStyleOptionButton::Flat)) {
        ...
        if (accent) {
          ...
          targetBgColor =
              controlFillBrush(option, ControlType::Control).color();
        }
        ...
        if (styleObject) {
          ...
          if (oldBtnState != currentBtnState) {
            QColor fromColor;
            if (QNumberStyleAnimation *prevAnim =
                    qobject_cast<QNumberStyleAnimation *>(
                        getAnimation(styleObject))) {
              const QColor prevFromColor =
                  styleObject->property("_q_btn_from_color").value<QColor>();
              const QColor prevTargetColor =
                  styleObject->property("_q_btn_target_color").value<QColor>();
              if (prevFromColor.isValid() && prevTargetColor.isValid()) {
                fromColor = blend(prevTargetColor, prevFromColor,
                                  prevAnim->currentValue());
              } else {
                fromColor =
                    prevFromColor.isValid() ? prevFromColor : targetBgColor;
              }
            } else {
              fromColor =
                  styleObject->property("_q_btn_from_color").value<QColor>();
        ...
        const bool defaultButton =
            btn->features.testFlag(QStyleOptionButton::DefaultButton);
        painter->setBrush(Qt::NoBrush);
        painter->setPen(defaultButton ? accentColor(option)
                                      : winUI3Color(controlStrokePrimary));
        painter->drawRoundedRect(rect, secondLevelRoundingRadius,
                                 secondLevelRoundingRadius);
        ...
        // the bottom part of the same rounded outline.
        {
          ...
          const QColor bottomStroke =
              defaultButton ? winUI3Color(controlStrokeOnAccentSecondary)
                            : winUI3Color(controlStrokeSecondary);
          const QRectF bottomClip(rect.left(), rect.bottom() - 1.0,
                                  rect.width(), 1.5);
          painter->setClipRect(bottomClip);
          painter->setPen(QPen(bottomStroke, 1.0));
          painter->setBrush(Qt::NoBrush);
      ...
      if (btn->features.testFlag(QStyleOptionButton::HasMenu)) {
        ...
        const bool isEnabled = !isDisabled(option);
        const auto indSize =
            proxy()->pixelMetric(PM_MenuButtonIndicator, btn, widget);
        const auto indRect = QRect(rect.right() - indSize - 2, rect.top(),
                                   indSize, rect.height());
        const auto vindRect =
  ...
  case CE_CheckBox:
    if (const QStyleOptionButton *btn =
      ...
      subopt.rect = subElementRect(isRadio ? SE_RadioButtonIndicator
                                   ...
                                   btn, widget);
      proxy()->drawPrimitive(isRadio ? PE_IndicatorRadioButton
                                     : PE_IndicatorCheckBox,
                             &subopt, painter, widget);
      subopt.rect = subElementRect(
          isRadio ? SE_RadioButtonContents : SE_CheckBoxContents, btn, widget);
      proxy()->drawControl(isRadio ? CE_RadioButtonLabel : CE_CheckBoxLabel,
                           &subopt, painter, widget);
      if (btn->state & State_HasFocus) {
        QStyleOptionFocusRect fropt;
        fropt.QStyleOption::operator=(*btn);
    ...
    if (const QStyleOptionButton *btn =
      ...
      if (!btn->icon.isNull()) {
        ...
        const auto pix = btn->icon.pixmap(
            btn->iconSize, QStyleHelper::getDpr(painter),
            (btn->state & State_Enabled) ? QIcon::Normal : QIcon::Disabled);
#else
        QPixmap pix = btn->icon.pixmap(
            btn->iconSize,
            (btn->state & State_Enabled) ? QIcon::Normal : QIcon::Disabled);
      ...
      if (!btn->text.isEmpty()) {
        proxy()->drawItemText(
            painter, textRect, alignment | Qt::TextShowMnemonic, btn->palette,
            btn->state & State_Enabled, btn->text, QPalette::WindowText);
      }
    }
    break;
  case CE_MenuBarItem:
  ...
  case CE_MenuItem:
    ...
    if (const auto *menuitem =
      ...
      const QRect rect = menuitem->rect.marginsRemoved(QMargins(
          menuItemHMargin, menuItemVMargin, menuItemHMargin, menuItemVMargin));
      if (act && dis == false) {
        // 实际绘制时，右边界会有1px，调整后可以覆盖掉这个
        QRectF r = QRectF(rect).adjusted(0, 0, 1, 0);

        QBrush selectColor = highContrastTheme
                                 ? menuitem->palette.brush(QPalette::Highlight)
                                 : QBrush(winUI3Color(subtleHighlightColor));
        QString selectedMenuItemBackground =
            qApp ? qApp->property("selectedMenuItemBackground").toString()
                 : QString();
      ...
      if (menuitem->menuItemType == QStyleOptionMenuItem::Separator) {
        ...
        painter->setPen(highContrastTheme
      ...
      if (!menuitem->icon.isNull()) {
        ...
        // PM_SmallIconSize
        QRect vRect(
            visualMenuRect(QRect(rect.x() + xOffset, rect.y(),
                                 menuitem->maxIconWidth - 4, rect.height())));
      ...
      if (!s.isEmpty()) { // draw text
        ...
        if (t >= 0 && menuitem->menuItemType != QStyleOptionMenuItem::SubMenu) {
          ...
          if (highContrastTheme) {
            ...
            penColor = menuitem->palette.color(
                dis ? QPalette::Disabled : QPalette::Active, QPalette::Text);
            if (!dis) {
              penColor.setAlpha(
                  percentToAlpha(60.63)); // fillColorTextSecondary
            }
          }
          painter->setPen(penColor);
        ...
        if (highContrastTheme && act) {
          ...
          penColor = menuitem->palette.color(
              dis ? QPalette::Disabled : QPalette::Current, QPalette::Text);
        }
        painter->setPen(penColor);
      ...
      if (menuitem->menuItemType ==
          QStyleOptionMenuItem::SubMenu) { // draw sub menu arrow
        ...
        QPoint tl(rect.right() - 2 * windowsArrowHMargin - contentItemHMargin,
                  rect.top() + yOfs);
  ...
  case CE_ItemViewItem: {
    if (const QStyleOptionViewItem *vopt =
      ...
      const auto p = proxy();
      QRect checkRect =
          p->subElementRect(SE_ItemViewItemCheckIndicator, vopt, widget);
      QRect iconRect =
          p->subElementRect(SE_ItemViewItemDecoration, vopt, widget);
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 9, 0)
      isDecorationColumn =
          vopt->features.testFlag(QStyleOptionViewItem::IsDecoratedRootColumn);
#else
      if (const QTreeView *tv = qobject_cast<const QTreeView *>(widget)) {
        int treeCol = tv->treePosition();
      ...
      const bool tableRowStrategy =
          tableView && !highContrastTheme && vopt->index.isValid() &&
      ...
      if (tableRowStrategy) {
        ...
        if (!rowSelected && tableView->viewport()) {
          const int hoverRow =
              tableView->viewport()->property("_q_table_hover_row").toInt();
        ...
        if (rowSelected || rowHovered) {
          ...
          if (rowSelected && tableView->currentIndex().isValid() &&
            ...
            editingCurrentCell =
                fw && fw != tableView && tableView->isAncestorOf(fw);
          }

          if (rowSelected && !editingCurrentCell) {
            const auto *model = tableView->model();
            const int totalCols =
                model ? model->columnCount(vopt->index.parent()) : 0;
      ...
      if (vopt->features & QStyleOptionViewItem::HasCheckIndicator) {
        ...
        proxy()->drawPrimitive(PE_IndicatorItemViewItemCheck, &option, painter,
                               widget);
      }
      ...
      if (iconRect.isValid()) {
        ...
        const bool isSettingsSpinItem =
            vopt->index.isValid() &&
            vopt->index.data(NavigationSettingsSpinRole).toBool();
        if (isNavigationTreeView && isSettingsSpinItem) {
          PainterStateGuard psg(painter);
          const QByteArray animKey =
              navigationSettingsAnimKey(itemTreeView, vopt->index);
          qreal iconAngle = 0.0;
          if (const QNumberStyleAnimation *animation =
                  qobject_cast<QNumberStyleAnimation *>(getAnimationEx(
                      const_cast<QTreeView *>(itemTreeView), animKey))) {
          ...
          const QString iconCode =
              vopt->index.data(NavigationIconRole).toString();
          if (!iconCode.isEmpty()) {
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
            const int supersample = 2;
            const int glyphSide = qMax(16, side * supersample);
            const QPixmap glyphPixmap = navigationGlyphPixmapCached(
                iconCode, glyphSide, vopt->palette.text().color());
            painter->setRenderHint(QPainter::SmoothPixmapTransform, true);
            painter->drawPixmap(drawRect, glyphPixmap,
                                QRectF(glyphPixmap.rect()));
#else
            ...
            vopt->icon.paint(painter, drawRect.toRect(),
                             vopt->decorationAlignment, mode, state);
          }
        } else {
      ...
      painter->setPen(highlightCurrent && highContrastTheme
                          ? vopt->palette.base().color()
      ...
      if (isNavigationTreeView) {
        const bool isIconMode =
            ...
            (itemTreeView &&
             itemTreeView->property("navigationIconMode").toBool());
        if ((vopt->state & State_Children) && !isIconMode) {
          ...
          const QByteArray animKey = QByteArrayLiteral("_q_nav_branch_anim_") +
                                     QByteArray::number(vopt->rect.y());
          const QNumberStyleAnimation *animation =
              qobject_cast<QNumberStyleAnimation *>(
                  getAnimationEx(const_cast<QWidget *>(widget), animKey));
          ...
          painter->translate(center);
          painter->rotate(angle);
          QRectF r(-arrowRectF.width() / 2.0, -arrowRectF.height() / 2.0,
                   arrowRectF.width(), arrowRectF.height());
          painter->drawText(r, Qt::AlignCenter, ChevronDownMed);
        }
  ...
  case CE_DockWidgetTitle:
    if (const QStyleOptionDockWidget *dwOpt =
      ...
      if (isFloating) {
        ...
        if (widget && widget->windowIcon().cacheKey() !=
...
int FluentUI3Style::styleHint(StyleHint hint, const QStyleOption *opt,
                              const QWidget *widget,
  ...
  case SH_TabBar_Alignment:
    if (widget && widget->property(TabBarStyleProperty).toInt() ==
                      TabBarStyle::Navigation) {
      return Qt::AlignLeft;
    }
    break;
...
QSize FluentUI3Style::sizeFromContents(ContentsType type,
...
#if QT_CONFIG(tabwidget)
      if (widget->parent() &&
          !qobject_cast<const QTabWidget *>(widget->parent()))
#endif
  ...
  case CT_MenuItem:
    if (const auto *menuItem =
      ...
      if (menuItem->menuItemType == QStyleOptionMenuItem::Separator) {
        ...
        constexpr int fluentMenuItemHeight =
            36; // WinUI3 menu item height, including 3px top and bottom margins
        height = height < fluentMenuItemHeight ? fluentMenuItemHeight : height;
        if (!menuItem->icon.isNull()) {
          int iconExtent =
              proxy()->pixelMetric(PM_SmallIconSize, option, widget);
          height = qMax(height,
                        menuItem->icon.actualSize(QSize(iconExtent, iconExtent))
                                .height() +
                            4);
        }
      }
      ...
      if (menuItem->menuItemType == QStyleOptionMenuItem::DefaultItem) {
        ...
        width += fmBold.horizontalAdvance(menuItem->text) -
                 fm.horizontalAdvance(menuItem->text);
      }
...
#if QT_CONFIG(spinbox)
  case CT_SpinBox: {
    if (const auto *spinBoxOpt =
            qstyleoption_cast<const QStyleOptionSpinBox *>(option)) {
      // Add button + frame widths
      // const qreal dpi       = QStyleHelper::dpi( option );
      const bool hasButtons =
          (spinBoxOpt->buttonSymbols != QAbstractSpinBox::NoButtons);
      const int margins = 8;
      ...
      const int frameWidth =
          spinBoxOpt->frame
              ? pixelMetric(PM_SpinBoxFrameWidth, spinBoxOpt, widget)
              : 0;

      contentSize +=
          QSize(2 * buttonWidth + 2 * frameWidth + 2 * margins, 2 * frameWidth);
    }
    break;
  }
#endif
#if QT_CONFIG(combobox)
  case CT_ComboBox:
    if (const auto *comboBoxOpt =
            qstyleoption_cast<const QStyleOptionComboBox *>(option)) {
      contentSize = QProxyStyle::sizeFromContents(
          type, option, size,
  ...
  case CT_ProgressBar:
    if (widget &&
        widget->property(ProgressBarStyleProperty).toInt() == ProgressBarRing) {
  ...
  case CT_RadioButton:
  case CT_CheckBox:
    if (const auto *buttonOpt =
            qstyleoption_cast<const QStyleOptionButton *>(option)) {
      const auto p = proxy();
      const bool isRadio = (type == CT_RadioButton);

      const int width = p->pixelMetric(isRadio ? PM_ExclusiveIndicatorWidth
                                               : PM_IndicatorWidth,
                                       option, widget);
      const int height = p->pixelMetric(isRadio ? PM_ExclusiveIndicatorHeight
                                                : PM_IndicatorHeight,
                                        option, widget);
      ...
      contentSize.setHeight(
          qMax(size.height(), height + 2 * contentItemHMargin));
    }
    break;
  ...
  case CT_ToolButton: {
    ...
    if (auto optionTbtn =
      ...
      if (optionTbtn->features & QStyleOptionToolButton::HasMenu &&
          optionTbtn->toolButtonStyle == Qt::ToolButtonTextBesideIcon) {
        const int mbi =
            proxy()->pixelMetric(PM_MenuButtonIndicator, option, widget);
        ...
        contentSize.rwidth() += mbi + 4;
                 ...
                 optionTbtn->toolButtonStyle == Qt::ToolButtonTextOnly) {
        const int mbi =
            proxy()->pixelMetric(PM_MenuButtonIndicator, option, widget);
        contentSize.rwidth() += mbi + 4;
      }
  ...
  case CT_ItemViewItem: {
    ...
    if (const auto *viewItemOpt =
      ...
      } else if (auto view = qobject_cast<const QTreeView *>(widget);
                 view && view->property("ItemHeight").toInt() > 0) {
        contentSize = QProxyStyle::sizeFromContents(type, option, size, widget);
  ...
  case CT_TabBarTab: {
    ...
    const int tabBarStyle =
        widget ? widget->property(TabBarStyleProperty).toInt() : 0;
    const bool isPivotStyle = tabBarStyle == TabBarStyle::Pivot_Grow ||
                              tabBarStyle == TabBarStyle::Pivot_Slide ||
    ...
    if (isPivotStyle) {
      ...
      const int pivotHeight = pivotTextTopPadding + contentVisualHeight +
                              pivotIndicatorTopGap + pivotIndicatorHeight +
    ...
    } else if (widget && widget->property(TabBarStyleProperty).toInt() ==
                             TabBarStyle::Segmented_WinUI3) {
      if (const auto *tab =
              qstyleoption_cast<const QStyleOptionTab *>(option)) {
        if (tab->text.isEmpty()) {
    ...
    if (widget && widget->property(TabBarStyleProperty).toInt() ==
      ...
      if (const auto *tab =
        ...
        if (isVerticalNavigation) {
          constexpr int indicatorWidth = 3;
          constexpr int indicatorContentGap = 2;
          constexpr int contentLeadingInset =
              indicatorWidth + indicatorContentGap;
          constexpr int contentTrailingInset = 8;
          constexpr int iconTextSpacing = 8;
          ...
          if (const auto *tabBar = qobject_cast<const QTabBar *>(widget)) {
            ...
            cacheParts << QString::number(tabBar->count())
                       << tabBar->font().toString();
            ...
            const QString storedCacheKey =
                tabBar->property("_q_navigation_tab_width_cache_key")
                    .toString();

            int cachedTextWidth =
                tabBar->property("_q_navigation_longest_text_width").toInt();
            int cachedIconWidth =
                tabBar->property("_q_navigation_widest_icon_width").toInt();
            ...
            if (storedCacheKey != cacheKey || cachedTextWidth <= 0) {
              ...
              if (!iconSize.isValid()) {
                const int iconExtent = proxy()->pixelMetric(
                    QStyle::PM_SmallIconSize, option, widget);
                iconSize = QSize(iconExtent, iconExtent);
              }

              for (int i = 0; i < tabBar->count(); ++i) {
                longestTextWidth = qMax(
                    longestTextWidth,
                    tab->fontMetrics.horizontalAdvance(tabBar->tabText(i)));

                const QIcon icon = tabBar->tabIcon(i);
                if (!icon.isNull()) {
                  const QSize actualIconSize =
                      icon.actualSize(iconSize, QIcon::Normal, QIcon::Off);
                  widestIconWidth =
                      qMax(widestIconWidth, qMax(16, actualIconSize.width()));
                }
              }

              const_cast<QTabBar *>(tabBar)->setProperty(
                  "_q_navigation_tab_width_cache_key", cacheKey);
              const_cast<QTabBar *>(tabBar)->setProperty(
                  "_q_navigation_longest_text_width", longestTextWidth);
              const_cast<QTabBar *>(tabBar)->setProperty(
                  "_q_navigation_widest_icon_width", widestIconWidth);
              cachedTextWidth = longestTextWidth;
              cachedIconWidth = widestIconWidth;
            }

            const int contentWidth = contentLeadingInset + cachedIconWidth +
                                     iconTextSpacing + cachedTextWidth +
                                     contentTrailingInset;
            contentSize.setWidth(contentWidth);
          } else {
            const int iconWidth =
                tab->icon.isNull() ? 0 : qMax(16, tab->iconSize.width());
            const int textWidth = tab->fontMetrics.horizontalAdvance(tab->text);
            const int contentWidth = contentLeadingInset + iconWidth +
                                     iconTextSpacing + textWidth +
                                     contentTrailingInset;
            contentSize.setWidth(contentWidth);
          }
...
int FluentUI3Style::pixelMetric(PixelMetric metric, const QStyleOption *option,
                                const QWidget *widget) const {
  int res = 0;
  ...
  case PM_SliderTickmarkOffset:
    ...
    if (const auto *slider =
            qstyleoption_cast<const QStyleOptionSlider *>(option)) {
      const int crossSize = slider->orientation == Qt::Horizontal
                                ? slider->rect.height()
                                : slider->rect.width();
  ...
  case PM_MenuButtonIndicator: {
    ...
    if (widget) {
      ...
      QFont f(assetFont);
      f.setPixelSize(qRound(fontSize * 0.9f) <= 0
                         ? 1
                         : qRound(fontSize * 0.9f)); // a little bit smaller
      QFontMetrics fm(f);
...
void FluentUI3Style::polish(QWidget *widget) {
  ...
  if (auto *progressBar = qobject_cast<QProgressBar *>(widget);
      progressBar && progressBar->property(ProgressBarStyleProperty).toInt() ==
                         ProgressBarRing) {
    const QVariant duration =
        progressBar->property(ProgressBarRingIndeterminateDurationProperty);
    ...
    const bool hasCustomTrackColor =
        palette.isBrushSet(QPalette::Active, QPalette::Mid) ||
  ...
  if (auto cb = qobject_cast<QComboBox *>(widget)) {
    ...
    if (!useQtCenteredPopup && !cb->inherits("ExComboBox") && !animatorObject) {
      ...
      cb->setProperty(cBPopupAnimatorProperty,
                      QVariant::fromValue<QObject *>(animator));
    }
  } else if (auto *menu = qobject_cast<QMenu *>(widget)) {
    QObject *animatorObject =
        menu->property(menuPopupAnimatorProperty).value<QObject *>();
    if (!animatorObject) {
      auto *animator = new MenuPopupAnimator(menu, menu);
      animator->setDuration(menuPopupAnimationDuration);
      menu->setProperty(menuPopupAnimatorProperty,
                        QVariant::fromValue<QObject *>(animator));
    }
  }
  ...
  if (auto tooBtn = qobject_cast<QToolButton *>(widget);
      tooBtn && (widget->objectName() == "ScrollLeftButton" ||
  ...
  if (isScrollBar) {
    ...
    if (scrollarea->frameShape() == QFrame::StyledPanel) {
      ...
      const bool isStyledBackground =
          vp->testAttribute(Qt::WA_StyledBackground);
  ...
  if (auto treeView = qobject_cast<QTreeView *>(widget);
    ...
    auto triggerBranchAnimation = [treeView](const QModelIndex &index,
      ...
      const QByteArray animKey =
          (isNavView ? QByteArrayLiteral("_q_nav_branch_anim_")
      ...
      QNumberStyleAnimation *existAnim = qobject_cast<QNumberStyleAnimation *>(
          getAnimationEx(treeView, animKey));
      qreal startAngle = existAnim ? existAnim->currentValue()
                                   : (opening ? closedAngle : openAngle);
  ...
  if (auto *buttonBox = qobject_cast<QDialogButtonBox *>(widget)) {
    static const QList<QDialogButtonBox::StandardButton> kAccentStdBtns = {
        QDialogButtonBox::Ok,    QDialogButtonBox::Yes,
        QDialogButtonBox::Save,  QDialogButtonBox::SaveAll,
        QDialogButtonBox::Open,  QDialogButtonBox::Apply,
...
void FluentUI3Style::unpolish(QWidget *widget) {
  ...
  if (auto *comboBox = qobject_cast<QComboBox *>(widget)) {
    ...
    auto *animator = static_cast<MenuPopupAnimator *>(
        menu->property(menuPopupAnimatorProperty).value<QObject *>());
  ...
  if (const auto *scrollarea = qobject_cast<QAbstractScrollArea *>(widget);
    ...
    const auto vp = scrollarea->viewport();
    const auto wasAutoFillBackground =
        vp->property("_q_original_autofill_background").toBool();
    vp->setAutoFillBackground(wasAutoFillBackground);
    vp->setProperty("_q_original_autofill_background", QVariant());
    const auto origStyledBackground =
        vp->property("_q_original_styled_background").toBool();
...
void FluentUI3Style::updateComboBoxAnimationEffect(QApplication *app) {
  ...
  const QVariant dropDownValue =
      app->property(ComboBoxPopupDropDownAnimationEnabledProperty);
  const bool dropDownAnimationEnabled =
      !dropDownValue.isValid() || dropDownValue.toBool();
...
QIcon FluentUI3Style::standardIcon(StandardPixmap sp,
                                   const QStyleOption *option,
                                   const QWidget *widget) const {
#if QT_VERSION > QT_VERSION_CHECK(6, 3, 0)
  if (sp == SP_LineEditClearButton || sp == SP_TitleBarCloseButton ||
      sp == SP_TabCloseButton)
#else
  if (sp == SP_LineEditClearButton || sp == SP_TitleBarCloseButton)
#endif
  {
    ...
    if (sp == SP_TitleBarCloseButton) {
      ...
      p.setFont(f);
      p.setPen(widget ? widget->palette().color(QPalette::Window)
  ...
  if (sp == SP_TitleBarNormalButton) {
    return colorSchemeIndex
               ? QIcon(":/resource/images/window-restore-s-light.png")
               : QIcon(":/resource/images/window-restore-s-dark.png");
  }

  auto createMsgBoxIcon = [this](const QChar &ch,
                                 const QColor &color) -> QIcon {
    QFont f(assetFont);
    f.setPixelSize(52);
  ...
  if (sp == SP_MessageBoxInformation) {
    return createMsgBoxIcon(QChar(0xF167), colorSchemeIndex
                                               ? QColor("#60CDFF")
                                               : QColor("#005FB8"));
  }
  if (sp == SP_MessageBoxWarning) {
    return createMsgBoxIcon(WarningMessage, colorSchemeIndex
                                                ? QColor("#FCE100")
                                                : QColor("#9D5D00"));
  }
  if (sp == SP_MessageBoxCritical) {
    return createMsgBoxIcon(QChar(0xEB90), colorSchemeIndex
                                               ? QColor("#FF99A4")
                                               : QColor("#C42B1C"));
  }
  if (sp == SP_MessageBoxQuestion) {
    return createMsgBoxIcon(Help, colorSchemeIndex ? QColor("#60CDFF")
                                                   : QColor("#005FB8"));
  }
...
void FluentUI3Style::drawCheckBox(const QStyleOption *option, QPainter *painter,
                                  const QWidget *widget) const {
  Q_UNUSED(widget)
...
void FluentUI3Style::drawSwitchButton(const QStyleOption *option,
                                      QPainter *painter,
                                      const QWidget *widget) const {
  Q_UNUSED(widget)

  const QStyleOptionButton *btn =
      static_cast<const QStyleOptionButton *>(option);
  ...
  float pos = animationValue(option->styleObject, "_q_thumb_pos",
                             (checked ? 1.0f : 0.0f));
  float scale = animationValue(option->styleObject, "_q_thumb_scale",
                               (hovered ? 1.1f : 0.9f));
  float stretch = animationValue(option->styleObject, "_q_thumb_stretch",
                                 (pressed ? 1.3f : 1.0f));
...
QBrush FluentUI3Style::controlFillBrush(const QStyleOption *option,
                                        ControlType controlType) const {
  using namespace StyleOptionHelper;
  static constexpr WINUI3Color colorEnums[2][4] = {
      // Light & Dark Control
      {fillControlDefault, fillControlSecondary, fillControlTertiary,
       fillControlDisabled},
...
QBrush FluentUI3Style::inputFillBrush(const QStyleOption *option,
                                      const QWidget *widget) const {
  // slightly different states than in controlFillBrush
  using namespace StyleOptionHelper;
...
QColor FluentUI3Style::controlTextColor(const QStyleOption *option,
  ...
  static constexpr WINUI3Color colorEnums[2][4] = {
      ...
      // Control, checked
      {textOnAccentPrimary, textOnAccentPrimary, textOnAccentSecondary,
...
void FluentUI3Style::drawLineEditFrame(QPainter *painter, const QRectF &rect,
                                       const QStyleOption *option,
                                       bool isEditable,
  ...
  if (qobject_cast<QLineEdit *>(option->styleObject)) {
    ...
    if (animation) {
      ...
      drawRoundedUnderline(painter, animRect, rect, roundingRadius,
                           QPen(focusColor, 2));
    } else {
      drawRoundedUnderline(painter, rect, rect, roundingRadius, penUnderline);
    }
...
void FluentUI3Style::drawPopupShadow(QPainter *painter, const QRectF &panelRect,
                                     int radius, int extent) const {
  ...
  const QColor baseColor = colorSchemeIndex == 1 ? QColor(0x00, 0x00, 0x00)
  ...
  for (int level = 1; level <= visibleLevels; ++level) {
    ...
    const QRectF outerRect = panelRect.adjusted(-levelExtent, -levelExtent,
    ...
    ringPath.addRoundedRect(outerRect, radius + levelExtent,
                            radius + levelExtent);
...
void FluentUI3Style::drawEffectShadow(QPainter *painter, QRect widgetRect,
  ...
  QColor color = colorSchemeIndex == 1 ? QColor(0x70, 0x70, 0x70)
...
void FluentUI3Style::drawSliderHandleShadow(QPainter *painter,
  ...
  if (auto *popup = qobject_cast<QWidget *>(watched);
    ...
    if (event->type() == QEvent::Show) {
      ...
      if (qApp && qApp->property("_q_scrollHint_center").toBool()) {
        ...
        QTimer::singleShot(0, popup, [popup] {
          if (!popup->isVisible() || !qApp ||
              !qApp->property("_q_scrollHint_center").toBool()) {
            return;
          }
    ...
    if (treeView && treeView->viewport() == viewport &&
      ...
      if (me->button() == Qt::LeftButton && transitionsEnabled()) {
        ...
        if (event->type() == QEvent::MouseButtonPress) {
          if (index.isValid() &&
              index.data(NavigationSettingsSpinRole).toBool()) {
            treeView->setProperty("_q_nav_settings_spin_pressed",
          ...
          const QModelIndex pressed =
              treeView->property("_q_nav_settings_spin_pressed").toModelIndex();
          treeView->setProperty("_q_nav_settings_spin_pressed", QModelIndex());
          if (pressed.isValid() &&
              pressed.data(NavigationSettingsSpinRole).toBool()) {
            startNavigationSettingsSpin(treeView, treeView, pressed, 360.0,
                                        600);
...
void FluentUI3Style::drawFluentDial(const QStyleOptionSlider *dial,
                                    QPainter *painter, const QWidget *widget,
                                    int style) const {
  PainterStateGuard psg(painter);
  painter->setRenderHint(QPainter::Antialiasing);
  ...
  if (dial->maximum > dial->minimum) {
    currentAngle = startAngle + spanAngle *
  ...
  if (drawValue) {
    ...
    painter->drawText(QRectF(center.x() - knobRadius, center.y() - knobRadius,
  ...
  if (style != DialThumb) {
    ...
    QPointF p1(center.x() + indicatorStart * qCos(radAngle),
               center.y() - indicatorStart * qSin(radAngle));
  ...
  if (style == DialDots) {
    ...
    for (int i = 0; i <= notches; ++i) {
      ...
      if (dial->upsideDown) {
        isActive =
            spanAngle < 0 ? (angle >= currentAngle) : (angle <= currentAngle);
      } else {
        isActive =
            spanAngle < 0 ? (angle <= currentAngle) : (angle >= currentAngle);
      }
      ...
      QPointF dotCenter(center.x() + trackRadius * qCos(dotRadAngle),
                        center.y() - trackRadius * qSin(dotRadAngle));
    ...
    painter->setBrush(Qt::NoBrush);
    QRectF trackRect(center.x() - trackRadius, center.y() - trackRadius,
                     trackRadius * 2, trackRadius * 2);
    painter->drawArc(trackRect, startAngle * 16, spanAngle * 16);

    qreal baseStartAngle =
        dial->upsideDown ? startAngle : (startAngle + spanAngle);
    ...
    qreal radAngle = currentAngle * M_PI / 180.0;
    QPointF thumbCenter(center.x() + thumbTrackRadius * qCos(radAngle),
                        center.y() - thumbTrackRadius * qSin(radAngle));
    ...
    if (widget) {
      const_cast<QWidget *>(widget)->setProperty(
          "_q_dial_thumb_rect",
          QRectF(thumbCenter.x() - outerRadius, thumbCenter.y() - outerRadius,
                 outerRadius * 2, outerRadius * 2));
    }

    bool isThumbHovered =
        widget ? widget->property("_q_dial_thumb_hovered").toBool() : false;
    bool isThumbPressed =
        widget ? widget->property("_q_dial_pressed").toBool() : false;
    ...
    float innerRadius =
        outerRadius * sliderInnerRadius(simulatedState, isThumbHovered);
    if (transitionsEnabled() && dial->styleObject) {
      ...
      bool doTransition =
          (((simulatedState & State_Sunken) != (oldState & State_Sunken)) ||
      ...
      if (doTransition) {
        ...
        t->setStartValue(styleObject->property("_q_inner_radius").toFloat());
        t->setEndValue(outerRadius *
                       sliderInnerRadius(simulatedState, isThumbHovered));
        styleObject->setProperty("_q_end_radius", t->endValue());
        t->setDuration(180);
        startAnimation(t);
      }
