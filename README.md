...
- **版本兼容性**：目前在Qt5.14.2、Qt5.15.2、Qt6.5.3、Qt6.6.3（MSVC环境）下测试正常
...
| Qt5.15.2 | ✅ 支持 | 已测试 |
| Qt6.6.3  | ✅ 支持 | 已测试 |
| Qt6.8+   | ⚠️ 部分 | Popup 菜单/下拉 **外阴影** 可能有 Bug（见上文说明） |
| Qt6.10   | ⚠️ 部分 | 同上；样式代码基于 Qt 6.10 Win11 样式移植 |
...
Example 的无边框窗口与 DWM 背景依赖 **[QWindowKit](https://github.com/stdware/qwindowkit)**，以 Git **子模块** 形式放在 `3rd/qwindowkit/`。  
`git clone` 默认不会拉取子模块内容，若缺少该目录会导致 CMake 配置失败。
...
> **注意：** 日常 `git pull` **不会**自动更新子模块；拉代码后若编译报找不到 `3rd/qwindowkit`，请再执行一次 `git submodule update --init --recursive`。
